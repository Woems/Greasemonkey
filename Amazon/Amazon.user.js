...
var l=["href", "protocol", "host", "hostname", "port", "pathname", "hash", "search"];
//alert(l.map(function (e) { return "location."+e+": "+eval("location."+e); }).join("\n"));



function CSSbyURL(url) { jQuery("<link rel='stylesheet' type='text/css'>").appendTo(document.getElementsByTagName("HEAD")[0])[0].href=url; }
function CSSbyRES(res) { CSSbyURL(GM_getResourceURL(res)); }
...
function Amazon()
{
  this.ASIN="";
  try {
  this.ASIN=(($xs('//li[b[contains(text(),"ASIN:")]]')||{textContent:""}).textContent.match(/[A-Z0-9]{10}/)||[""])[0];
  } catch(e) { alert("ASIN: "+e); }
...
if (!inFrame())
{
  ...
  jQuery("#az-status").text(AZ.ASIN + ": " + AZ.getStatus());
  ...
  jQuery("#az-menu button").click(function (e) { AZ.setStatus(jQuery(this).text()); jQuery("#az-status").text(AZ.getStatus()); });

}

} catch(e) { alert(uneval(e)); }
