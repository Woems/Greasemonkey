...
{
  case 'www.9kw.eu': case '9kw.eu': _9kw(); break;
  default: if (alleXTage(1)) GM_openInTab("http://9kw.eu"); break;
}
...
function _9kw()
{
  ...
  window.setTimeout(function () { 
    if ($("search_form") &&
        $("search_form").elements.namedItem('email').value!="" &&
        $("search_form").elements.namedItem('pass').value!="")
      $("search_form").elements.namedItem('search').click();
  }, 1*1000); 
  //alert([$("search_form"), $("search_form").elements.namedItem('email').value, $("search_form").elements.namedItem('pass').value, $("search_form").elements.namedItem('search').value].join("\n"));

  // Guthabendetektor
  if ($("guthaben"))
  {
    var Guthaben=$("guthaben").textContent.split(" ")[1]*1;
    if (Guthaben<500) alert("Das Guthaben bei 9kw beträgt nur noch '"+Guthaben+"' Punkte.");
  }
}
