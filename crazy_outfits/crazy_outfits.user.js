...
// @name           Crazy Outfits
// @namespace      Woems
// @include        http://www.crazy-outfits.de/shop/*
// ==/UserScript==
...
// XHTML
function get(url, cb) { GM_xmlhttpRequest({ method: "GET", url: url, onload: function(xhr) { cb(url, xhr.responseText, xhr.responseHeaders); } });} // get("http://google.de", function (url, text, haeder) {});
...
function text2div(text,elementtype) { var div=document.createElement(elementtype||"div"); div.innerHTML=text; return div; }
// Text
...
$x("//table[@class='produkt']//a/img").forEach(function (a) {
  a.addEventListener('mouseover',function (event) {
    //GM_log("hover");
    var div=getDiv("BigImage");
    div.style.display="";
    //div.style.color="white";
    css(css(div,"color","black"),"backgroundColor","lightgray");
    //if (PosXY(a).y+a.height > document.height)
    //css(css(css(div,"position","absolute")
    //               ,"top",(PosXY(a).y-div.height)+"px")
    //               ,"left",(PosXY(a).x)+"px");
    //else
    css(css(css(div,"position","absolute")
                   ,"top",(PosXY(a).y+a.height)+"px")
                   ,"left",(PosXY(a).x)+"px");
    div.innerHTML="Bild bitte warten";
    get(event.target.parentNode.href,function (url, text, haeder) {
      var div=text2div(text);
      $("BigImage").innerHTML="";
      $x("//a[@target='Produktbild']",div).forEach(function (a) {
        $("BigImage").innerHTML+="<img src='"+a.href+"' width=200>";
      });
    });
...
$x("//a[@target='Produktbild']").forEach(function (a) {
  a.addEventListener('mouseover',function (event) {
    var div=getDiv("BigImage");
    div.style.display="";
    div.style.position="absolute";
    div.style.top=(PosXY (event.target).y+event.target.height)+"px";
    div.style.left=(PosXY(event.target).x)+"px";
    div.innerHTML="<img src="+event.target.parentNode.href+">";
  }, true);
  a.addEventListener('mouseout',function (event) {
    var div=getDiv("BigImage");
    div.style.display="none";
