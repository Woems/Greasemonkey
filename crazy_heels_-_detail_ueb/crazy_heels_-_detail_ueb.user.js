...
switch (loc[1])
{
  ...
  case "advanced_search_result": Index(); break;
...
function Index()
{
  ...
  $x("id('wlist')//a[img]").forEach(function (f) {
    ...
    if (tmp)
    {
      if (tmp["farbe"]) insertBefore(createElement("div",{ innerHTML:"Farbe: "+tmp["farbe"] }),$xs("following::td[2]/text()",f));
      if (tmp["absatz"]) insertBefore(createElement("div",{ innerHTML:"Absatz: "+tmp["absatz"] }),$xs("following::td[2]/text()",f));
      if (tmp["size"]) insertBefore(createElement("div",{ innerHTML:"Size: "+tmp["size"] }),$xs("following::td[2]/text()",f));
    } //else GM_openInTab(f.href);
  });
}
...
function Product()
{
  ...
  $xs("id('cart_quantity')//td[@class='produktbeschreibung']/p").id="wcolor";
...
try {
  data[location.href]={
    ...
    farbe:       ($("wcolor").innerHTML.replace(/\s+/g," ").match(/Farbe: *(.+) *(<br>|Absatz:)/)||[false,false
])[1],
