...
function showmsg(data)
{
  ...
  if (data.onCancel) data.cancelbtn=createElement("input",{ type:"button", value:data.Cancel||"Abbrechen", style:"margin:0px 0px 0px 4px;", onClick:function () { data.onCancel(data); remove($(data.id));  } }, data.box);
...
if (isShinken())
{
  ...
  followLinks();
  showmsg({
    text:'Status: <select name="status"><option>Gut</option><option>Schlecht</option></select><input name="Test">',
    fixed:true,
    color:'lightgray',
    onOK:function (e) { alert(e["status"].value+e["Test"].value); },
    onCancelTimeout:function (e) {},
    Timeout:60,
  });
}

function HiddenMenu() {
  GM_registerMenuCommand("Shinken",function () { alert(setShinken("toggle")?"an":"aus"); });
}

function isShinken(loc) {
  var data=deserialize('data',{});
  return (data[(loc||location).hostname]||{}).shinken;
}

function setShinken(onofftoggle,loc) {
  if (!loc) loc=location;
  var data=deserialize('data',{});
  if (!data[loc.hostname]) data[loc.hostname]={};
  data[loc.hostname].shinken={ undefined:false, true:true, false:false, "on":true, "off":false, "toggle":!data[loc.hostname].shinken}[onofftoggle];
  serialize('data',data);
  return data[loc.hostname].shinken;
}

function followLinks() {
  $x("//a[@href]").forEach(function (a) { a.addEventListener("click",function(event){ if (isShinken(a)==undefined) setShinken(confirm("ist "+a.hostname+" shinken?"),a);  }, true); });
}
