...
// @name         Google Fx
// @version      2.1.5
// @description  Nice Look&Feel + Sites ThumbShots + Global Google Suggest (multi-lang) + Full AutoPage Results + Easy Access to Advance Filters + Relink Images & FullSize preview + Search in Other Searchers + Static Menus & ToolBars + Multi-Language Feature + Full User's Settings... and more. [Op & FF]
// @date         2009-04-18
...
CSS = {_ver		: SCRIPT_VER,
	...
	res:{
		...
		-------------------------------------------*/
		box:			getValue('CSS.res.box', 'border-color:#c9d7f1; border-style:solid; border-width:1px; margin-bottom:2px; margin-left:8px; margin-right:5px; margin-top:2px; padding-bottom:5px; padding-left:8px; padding-right:5px; padding-top:5px;'),

		/** Titles of Results
		-------------------------------------------*/
		tit:			getValue('CSS.res.tit', 'font-family:"Trebuchet MS", Georgia, serif; font-weight:normal; font-size:14pt; margin-bottom:1px;'),
...
selectAllNodes = function (xpath, context, target)
{
	// XPathResult.ORDERED_NODE_SNAPSHOT_TYPE = 7
	var nd = (target||document).evaluate(xpath, (context||doc.body), null, 7, null);
	var r = [];
	for (var i=0, len=nd.snapshotLength; i < len; i++) {
		r[i] = nd.snapshotItem(i);
	}
	nd = null;
	return r;
};

selectNode = function (xpath, context, target)
{
	// XPathResult.FIRST_ORDERED_NODE_TYPE = 9
	return (target||document).evaluate(xpath, (context||doc.body), null, 9, null).singleNodeValue;
};
...
QUERYS = {};
if(URI.query.q && !isGoogleCache)
{
	var q = URI.query.q.replace(/\+/g,' ');
	while ( q.lastIndexOf(':') != -1 )
	{
		var name, val;
		name = q.split(/([\-]?\s*[a-zA-Z]+\s*:\s*[^ ]*)/);
		q = name[0];
		if(!name[1] || q.length < 3){continue;}
		name = name[1].split(':');
		val = name[1].replace(/^\s+/,'').replace(/\s+$/,'');
		name = name[0].replace(/\s/g,'').replace(/\-/,'_');
...
if(!isByRequest){
	titlesSize = CSS.res.tit.match(/font-size\s?:([0-9]+)/i);
	titlesSize = ( titlesSize && titlesSize[1] ) ? Number(titlesSize[1]) : 12;
}
...
var GFX_FirstRun = function()
{
	...
	// Change doc variable in case of Gmail because use iframes
	/*
	if(SERVICE == 'mail')
	{
		doc = URI.query.ui == '1' ?  selectNode("//iframe[@id='v1']",window.frames[0].document.body, window.frames[0].document).contentDocument : selectNode("//iframe[@id='canvas_frame']").contentDocument;
	}
	*/
	...
	*************************/
	...
	{
		var q = encodeURL((body.getElement('INPUT[name=q]')||'').value);
		
		var type = $('gfx-searchers').firstChild.value;
	...
	var addSearchers = function (target, type)
	{
		...
		itemBar.set('onclick'		, 'gbar.open(event,"gfx-searchers-menu");return false;');
		itemBar.set('onmouseover'	, 'gbar.open(event,"gfx-searchers-menu");return false;');
		...
		for(var name in SEARCHER){
			menuHolder.appendChild(new Element('a', {
				'class'		: name,
				'href'		: '#',
				'html'		: LNG[name],
				'title'		: LNG[name],
		...
		var comboBox = new Element('input', {
			...
			'events'	: {
				'click'	:function(ev){
					var frm = body.getElement('FORM');
					var btn = body.getElement('INPUT[type=submit]');
		...
		GMENU.appendChild(itemBar);
		GMENU.appendChild(menuHolder);
	...
	var AutoPager = function(obj){
		...
		var onTimeOut = function()
		{
			...
			var isLoaded = $(contentDocument.body).getElement('var[id=gfx-req-loaded]');
				...
				if(contentNodes)
				{
					...
					if(isReverse){
						...
						do {
							...
							$(iNode).inject(holder, 'top');
						} while (len--);

						if(GFX.rAutoPagNav){
							$(holder).grab(new Element('a',{'name':'pos_'+prevStart}),'top');
							$(navContent).grab(new Element('a',	{
								'href'	: '#pos_'+prevStart,
								'html'	: '<span class="csb ch"></span>'+(Math.round(prevStart/10)+1)
							}),'top');
							navContent.getParent().scrollTo(0,0);
						}
						...
						WinScroller.start(0, window.getSize().y);
					...
					// Next Page
						...
						if(GFX.rAutoPagNav){
							$(holder).grab(new Element('a',{'name':'pos_'+nextStart}));
							navContent.grab(new Element('a',	{
								'href'	: '#pos_'+nextStart,
								'html'	: '<span class="csb ch"></span>'+(Math.round(nextStart/10)+1)
							}));
							navContent.getParent().scrollTo(9999999,0);
						}
					
						var nextLink = selectNode(xPathNext, contentDocument.body, contentDocument);
						
						if( !nextLink || nextStart > Number(nextLink.href.match(/start=([0-9]+)/)[1]) )
						{
							$(top.window).removeEvent('scroll', onScroll);
							var nofilter = $(contentDocument.body).getElement('A[href*="filter=0"]');
							if(nofilter){
		...
		var init = function ( obj )
		{
			...
			holder = typeof(obj.holder)=='string' ? selectNode(obj.holder, doc.body, doc) : obj.holder;
			...
			var nav = body.getElement('*[id*=nav]');
			if(nav){ nav.hide(); }
			
			var next = selectNode(xPathNext, doc.body, doc);
			...
			if(GFX.rAutoPagNav){
				body.grab(new Element('a',{'name':'pageTop'}),'top');
				...
				var step = window.getScrollSize().y / len;
			...
			$(top.window).addEvent('scroll', onScroll);
	...
	var addGUI = function()
	{
		...
		<div id=gfx-gui-dragger></div>\
		  <div class=header>\
			<span>\
			  ...
			  <ul id=tabmenu>\
				<li><a href=# onclick=setTabMnu(0,event)>' + LNG.general + '</a></li>\
				<li><a href=# onclick=setTabMnu(1,event)>' + LNG.toolbar + '</a></li>\
				<li><a href=# onclick=setTabMnu(2,event)>' + LNG.results + '</a></li>\
				<li><a href=# onclick=setTabMnu(3,event)>' + LNG.images + '</a></li>\
				<li><a href=# onclick=setTabMnu(4,event)>' + LNG.styles + '</a></li>\
				<li><a href=# onclick=setTabMnu(5,event)>' + LNG.about + '</a></li>\
			  </ul>\
			</span>\
		    <span class=subtitle>\
			  <h1>' + LNG.ui_title + '</h1>\
			  <a href="'+	SCRIPT_URL +'" target=_blank>' + LNG.ui_chk_upd + '</a>\
		     </span>\
		  </div>\
		  <div class=tabs>\
		...
		html += '	</div>\
			...
			<fieldset id=gfx-tab-2>\
				...
				<label><input name=GFX.rThShots type=checkbox \
					onclick="$(\'gfx-preview-tn\').style.display=(this.checked?\'inline-block\':\'none\'); var o=this.parentNode.nextSibling; o.className = o.nextSibling.className = (this.checked ? \'sub\' : \'sub disabled\');" '
				+ (GFX.rThShots ? 'checked=checked' : '')
				+ '>' + LNG.ui_res_thumbs + '</label>\
				<label class=sub><input name=GFX.rThLft type=checkbox onclick="var o = $(\'gfx-preview-tn\'); o.style.float = (this.checked ? \'left\' : \'right\');" '
				+ (GFX.rThLft ? 'checked=checked' : '')
				+ '>' + LNG.ui_res_th_lft + '</label>\
				<label class=sub>' + LNG.ui_res_th_sz + ': '	+ '\
				<select name=GFX.rThSize onchange="var o=$(\'gfx-preview-tn\');var v=this.options[this.selectedIndex].value;o.style.minWidth=(120*v)+\'px\';o.style.minHeight=(90*v)+\'px\';o=$(\'PV_content_holder\');o.style.minHeight=((90*v)+10)+\'px\';">'	+ '\
				...
				<label><input name=GFX.rCounter type=checkbox \
					onclick="var o = $(\'PV_res_ctn\'); o.style.display = (this.checked ? \'inline-block\' : \'none\');" ' 
				...
				+ ' onclick="this.parentNode.nextSibling.className = (this.checked ? \'sub\' : \'sub disabled\'); var o = $(\'PV_res_ref\'); o.innerHTML = (this.checked ? \'<a class=ch></a><a class=sm></a>\' : \'<a>in Cache</a> - <a>Similar pages</a>\'); $(\'PV_res_ref\').className = (this.checked ? \'res_ref repos\' : \'res_ref\');"'
				+ '>' + LNG.ui_res_ref_icon + '</label>\
				<label class=sub><input name=GFX.rRefIcnRgt type=checkbox '
				+ ' onclick="var o = $(\'PV_res_ref\'); o.className = (this.checked ? \'res_ref repos\' : \'res_ref\');"'
				...
				+ ' onclick="var o=$(\'PV_res_url\');o.innerHTML=(this.checked?\'<a class=qs></a>\':\'\')+\'www.domain.url - 99k\';"'
			...
			<fieldset id=gfx-tab-4>\
				...
				<select id=gfx-css-list>\
				  ...
				  <option id=CSS.cmn.btnsOv>'	+ LNG.buttons		+' - [MouseOver]</option>\
				...
				<table id=gfx-css-styles\
				<tr>\
				<td>\
					Font-Family:\
				</td><td colspan=3>\
					<input id=CSS_font-family type=text size=43 onblur="closeList()"><span onclick="addList(this,\'Arial,Arial Black,Comic Sans MS,Courier New,Georgia,Impact,Times New Roman,Trebuchet MS,Verdana\');" class=virtual-list-icon>&#9660;</span>\
				</td>\
				</tr><tr>\
				<td>\
					Font-Size:\
				...
				<td>\
					...
					<input id=CSS_text-align type=text size=8 pattern="\s*(left|center|right|justify)\s*" onblur="closeList()"><span  onclick="addList(this,\'left,center,right,justify\');" class=virtual-list-icon>&#9660;</span>\
				</td><td>\
					White-Space:\
				</td><td>\
			...
			<fieldset id=gfx-tab-5>\
				<legend>' + LNG.about + '</legend>\
				<div class=about>\
				<span>\
				<img src="/intl/en/logos/Logo_40wht.gif" width=128 height=53 style="border:0;margin:0;" />\
				<a class=gfx href="'+ SCRIPT_URL +'" target=_blank>v'+ SCRIPT_VER +'</a>\
				<span>\
				<br><a href="'+ SCRIPT_URL +'" target=_blank>'+ SCRIPT_URL +'</a>\
				<br><br>\
				<table>\
				<tr>\
					<td><b>Script Code:</b></td><td>Pablo Custo</td>\
				</tr><tr>\
					<td valign=top><b>Translators:</b></td><td valign=top>\
					Polish by Mr.Cat<br>\
					Dutch by Jerone<br>\
					</td>\
				</tr><tr>\
					<td valign=top><b>AddOn\'s Apps:</b></td><td valign=top>\
					GoogleFx Prefs by Mr.Cat\
					</td>\
				</tr><tr>\
					<td valign=top><b>Thanks:</b></td><td valign=top>\
					UserScripts.org<br>\
					Users Script\'s & UserScript Fans<br>\
					</td>\
				</tr>\
				</table>\
				</div>\
		...
		<div id=gfx-preview>\
			...
			<div id=PV_res_box class=res_box onmousedown=cssOp(this)>\
				...
				<div id=PV_res_hld>\
					...
					<span class=res_tit>\
						<a href="#" id=PV_res_tit onclick=cssOp(this)>\
							Title of </a>\
						<a href="#" id=PV_res_hl onclick=cssOp(this)>\
							<em class=res_hl>\
								results\
							</em>\
						</a>\
			...
			<br>\
			<div>\
				<input id=PV_cmn_intxt class=cmn_intxt onclick=cssOp(this) type=text size=9 value="Search...">\
				<input id=PV_cmn_btns class=cmn_btns onclick=cssOp(this) type=submit value=" Normal ">\
				<input id=PV_cmn_btnsOv class=cmn_btnsOv onclick=cssOp(this) type=submit value=" MouseOver ">\
			</div>\
		  </fieldset>\
		  </div>\
		  <div class=btns>\
		...
		$('gfx-gui-close').addEvent('click', hideSettings);
		$('gfx-gui-save').addEvent('click', saveSettings);
	...
	var makeSettings = function(event)
	{
		...
		if(win.setTabMnu){ win.setTabMnu(0); }

		var dragGUI = $('gfx-gui').makeDraggable({
			handle: $('gfx-gui-dragger')
		});
		
		var cssList = $('gfx-css-list');
		var cssPreview = $('gfx-preview');
		var cssStyleList = body.getElements('INPUT[id^=CSS_]');
		var cssValue = $('gfx-css-value');
		...
		{
			// Clean Values
			$each(cssStyleList,function(item){item.value='';});
			...
			curPrev.set('style', styles);
			if(lastPrev){ lastPrev.style.border=''; }
			lastPrev = curPrev;
			curPrev.style.border = '1px solid #c00';
			$each(styles.split(/\s*;\s*/), function(item){
				var v = item.split(/\s*:\s*/);
				var el = $( 'CSS_' + v[0] );
				if(el){ el.value = v[1]; }
			});
		};
		...
		var resetValues = function(ev)
		{
			cIdx = cssList.selectedIndex;
			var name = cssList.options[cIdx].id;
			AlertBox.confirm(LNG.ui_css_defaults+': <b>' + name.substring(4).toUpperCase() + '</b>?', {
				onComplete:
					function(isOk) {
						if (isOk) {
							GM_setValue(name, '');
		...
		$('gfx-css-reset').addEvent('click', resetValues);
		...
		var setPreview = function(ev)
		{
			...
			var styles = '';
			$each(cssStyleList, function(item){
				if(!/[a-z0-9\.\-]/.test(item.value)){ return; }
				styles += item.id.replace(/^CSS_/,'') + ':' + item.value + ';';
			});
			cssValue.value = styles;
			var name = cssList.options[cIdx].id.replace(/CSS/, 'PV').replace(/\./g, '_');
			$(name).set('style', styles.replace(/[a-z0-9\.\-]+:null;/g, ''));
		};
		
		$each(cssStyleList, function(item){
			$(item).addEvent('blur',setPreview);
			$(item).addEvent('keyup',setPreview);
		});
		
		BgBox.show();
		gui.show();
		gui.style.left = Math.round((doc.body.clientWidth-780)/2) + 'px';
		gui.style.top = '20px';
	};
	...
	var saveSettings = function(event)
	{
		var gui, o, x;
		gui = $('gfx-gui');
		var inputs = gui.getElements('INPUT[name^=GFX.]');
		$each(inputs, function(item){
			var value = (item.type=='checkbox' ? item.checked : item.value);
			GM_setValue(item.name, value);
		});

		var selects = gui.getElements('SELECT[name^=GFX.]');
		$each(selects, function(item){
			var idx = item.selectedIndex;
			GM_setValue(item.name, item.options[idx].value);
		});
	...
	if(GFX.gFxLogo)
	{
		var target = body.getElement('IMG[src*="/images/hp2.gif"]') || body.getElement('IMG[src*="/nav_logo"]') || body.getElement('A[id="logo"]') || $(selectNode("//img[@width='150' or contains(@alt,'Google') or contains(@src,'/intl/')][contains(@src,'/images/') or contains(@src,'logo')]")) || body.getElement('DIV[style*="/intl/"][style*="/images/"][title*="Google"]') || body.getElement('IMG[src*="/logos/"]') || body.getElement('DIV[id=":rc"]') || body.getElement('TABLE[id="frame"] DIV[title="Google"]');
		if(target) {
			...
			if(target.getSize().x < 170){
				...
				if(target.nodeName == 'IMG') {
					target.getParent().getParent().setStyle('min-width', '188px');
					logo.inject(target.getParent(),'before');
					
				} else {
					target.getParent().setStyle('min-width', '185px');
	...
	var HEADER = $('header') || body.getElement('#header');
	var GMENU = $('gbar');
	...
	if (!GMENU && GFX.bMnuChk)
	{
		GMENU = new Element('DIV',{
			'id' 	: 'gbar',
			'html'	: '\
			<nobr>\
			<a href="http://www.google.com/webhp?" class="gb1 search">'		+LNG.webhp+		'</a>\
			<a href="http://images.google.com/imghp?" class="gb1 images">'	+LNG.images+	'</a>\
			...
			<a href="http://www.google.com/intl/'+ LANG 
				+'/options/" onclick="gbar.open(event,\'gbi\');return false;" onmouseover="gbar.open(event,\'gbi\');return false;" class="gb3 intl">'
				+ LNG.more +' &#9660;</a>\
			<div id=gbi>' + gbimenu + '</div></nobr><div class=gbh style=left:0></div><div class=gbh style=right:0></div>'
		});
		GMENU.inject(body, 'top');
	...
	else if(GFX.bMnuChk)
	{
		...
		var addLink2Nav = function (obj, href, label, class)
		{
		...
		var cloneBar = GMENU.clone(true, true);
		...
		if(!gbi){
			...
			cloneBar.appendChild(a);
			gbi = new Element('DIV',{
				'id'	: 'gbi',
				'html'	: gbimenu,
			});
			cloneBar.appendChild(gbi);
		}

		var items = '';
		var nodes	= cloneBar.getElements('A');
		var lastGBar = cloneBar.getElement('A[class="gb3"][href*="/options/"]')||cloneBar.getLast('A[class="gb1"]');
		...
		var lastGbi = nodes.getLast();

		// Add Links to NavBar
		if(SERVICE != 'video'		&& items.indexOf('video')		== -1)
			addLink2Nav(lastGBar, 'http://video.google.com/videosearch?',	LNG.video,		'gb1 video');
		if(SERVICE != 'translate'	&& items.indexOf('translate')	== -1)
			addLink2Nav(lastGBar, 'http://translate.google.com/translate_t?',LNG.translate,	'gb1 translate');


		//Add Links to NavBar -> More..
		if(SERVICE != 'alerts'		&& items.indexOf('alerts')		== -1)
			addLink2Nav(lastGbi, 'https://www.google.com/alerts?',			LNG.alerts,		'gb2 alerts');
		if(SERVICE != 'blogsearch'	&& items.indexOf('blogs')		== -1)
			addLink2Nav(lastGbi, 'http://blogsearch.google.com/?',			LNG.blogs,		'gb2 blogsearch');
		if(SERVICE != 'books'		&& items.indexOf('books')		== -1)
			addLink2Nav(lastGbi, 'http://books.google.com/bkshp?',			LNG.books,		'gb2 books');
		if(SERVICE != 'calendar'	&& items.indexOf('calendar')	== -1)
			addLink2Nav(lastGbi, 'https://www.google.com/calendar/render?', LNG.calendar,	'gb2 calendar');
		if(SERVICE != 'code'		&& items.indexOf('code')		== -1)
			addLink2Nav(lastGbi, 'http://code.google.com/intl/'+LANG+'/'
			+ (hasSomeQuery ? '#q='+queryFormEncode : ''),					LNG.code,		'gb2 code');
		if(SERVICE != 'docs'		&& items.indexOf('docs')		== -1)
			addLink2Nav(lastGbi, 'https://docs.google.com/?',				LNG.docs,		'gb2 docs');
		if(SERVICE != 'finance'		&& items.indexOf('finance')		== -1)
			addLink2Nav(lastGbi, 'http://finance.google.com/finance?',		LNG.finance,	'gb2 finance');
		if(SERVICE != 'groups'		&& items.indexOf('groups')		== -1)
			addLink2Nav(lastGbi, 'http://groups.google.com/grphp?',			LNG.groups,		'gb2 groups');
		if(SERVICE != 'history'		&& items.indexOf('history')		== -1)
			addLink2Nav(lastGbi, 'https://www.google.com/history/lookup?hl='+LANG, LNG.history, 'gb2 history');
		if(SERVICE != 'maps'		&& items.indexOf('maps')		== -1)
			addLink2Nav(lastGbi, 'http://maps.google.com/maps?',			LNG.maps,		'gb2 maps');
		if(SERVICE != 'picasaweb'	&& items.indexOf('picasaweb')	== -1)
			addLink2Nav(lastGbi, 'http://picasaweb.google.com/lh/view?q='
			+ queryFormEncode + '&psc=G&filter=0',							LNG.picasaweb,	'gb2 picasaweb');
		if(SERVICE != 'products'	&& items.indexOf('/pr')			== -1)
			addLink2Nav(lastGbi, 'http://www.google.com/products?',			LNG.products,	'gb2 products');
		if(SERVICE != 'reader'		&& items.indexOf('reader')		== -1)
			addLink2Nav(lastGbi, 'https://www.google.com/reader/view/?',	LNG.reader,		'gb2 reader');
		if(SERVICE != 'sites'		&& items.indexOf('sites')		== -1)
			addLink2Nav(lastGbi, 'https://sites.google.com/?',				LNG.sites,		'gb2 sites');
		if(SERVICE != 'scholar'		&& items.indexOf('scholar')		== -1)
			addLink2Nav(lastGbi, 'http://scholar.google.com/scholar?',		LNG.scholar,	'gb2 scholar');
		if(URI.host[1] != 'youtube' && items.indexOf('youtube')		== -1)
			addLink2Nav(lastGbi, 'http://www.youtube.com/results?search_query='
			+ queryFormEncode + '&search_type=&aq=f',						LNG.youtube,	'gb2 youtube');

		cloneBar.replaces(GMENU);
		GMENU = cloneBar;

		lastGBar.set('onclick','gbar.open(event,"gbi");return false;');
		lastGBar.set('onmouseover','gbar.open(event,"gbi");return false;');
	...
	if(GMENU)
	{
		...
		var nodes = selectAllNodes("//div[@id='gbar']/descendant::a[@href][not(contains(@href,'picasaweb.') or contains(@href,'youtube.') or contains(@href,'mail.') or contains(@href,'/options') or contains(@href,'code.') or contains(@href,'/history'))][not(contains(@class,'gb3'))]", doc.body, doc);

		for (var i=0,len=nodes.snapshotLength; i < len; i++) {
			var el = nodes.snapshotItem(i);
			item.href = item.href.split('?')[0] + '?' + query;
		};
		...
		if(GFX.bShtName)
		{
			...
			{
				var guser = selectNode("//DIV[@id='gbar']/following::*/descendant::a[contains(@href,'accounts')]");
				if(guser){
					guser.setAttribute('class', (guser.getAttribute('class') || '') + ' login');
					var uname = $(guser.parentNode).getElement('B');
					if(uname && uname.innerHTML.indexOf('@') != -1)
						uname.innerHTML = uname.innerHTML.replace(/@[a-z]+\.com/,'');
				}
			}
	...
	//***********************************
	var FORM = $(doc.forms[0]) || body.getElement('FORM');
	...
	if(canRun.search)
	{
		...
		if(GFX.bMnuFix)
		{
			STYLES += '\
			BODY {\
				margin-top			: '+ (HEADER.getSize().y+5) +'px !important;\
			}\
			';
		...
		//-----------------------------------
		addSearchers((HEADER || body.getElement('div[id=res]')), SERVICE);
		...
		if(GFX.bAdvSrc)
		{
			...
			//Search by Date
			var byDate = body.getElement('INPUT[name="as_qdr"]');
			if (!byDate) {
				byDate = new Element('INPUT',{
					'name'	: 'as_qdr',
					'type'	: 'hidden'
				});
				(FORM||doc.body).appendChild(byDate);
			} else {
				STYLES += '\
				select[name=as_qdr] {\
					display			: none;\
			...
			//Search in
			var searchIn = body.getElement('INPUT[name="as_occt"][type="hidden"]');
			if (!searchIn)
			{
				searchIn = new Element('INPUT',{
					'name'	: 'as_occt',
					'type'	: 'hidden'
				});
				(FORM||doc.body).appendChild(searchIn);
			}

			var value = (URI.query.as_occt ? URI.query.as_occt : 'any');
			searchIn.value = value;
		...
		if(GFX.bRgtBox) {
			...
			// Implementation of the Right Panel
			//--------------------------------------------
			...
			var bRgtBox = doc.createElement('div');
			bRgtBox.id = 'gfx-rpanel';
			doc.body.appendChild(bRgtBox);
			...
			// Calc Panel
			var calcPanel = function()
			{
				var node = selectNode('//*[@id="res"]/descendant::table/descendant::img[@src="/images/calc_img.gif"]/ancestor::table');
				if (!node) return;
				var panel;
				var holder = $('gfx-rpanel');
				if( !(panel=$('gfx-rpanel-calc')) )
				{
					panel = doc.createElement('div');
					panel.id = 'gfx-rpanel-calc';
					panel.className = 'calc-Rp';
					var title = doc.createElement('h3');
					title.className = 'r';
					title.innerHTML = LNG.calc;
			...
			{
				var node = selectNode('//*[@id="res"]/descendant::form[@action="/movies"]/ancestor::li');
				if (!node) return;
				node.className += ' movieBrd';
				STYLES += '\
				.movieBrd {\
					'+ CSS.res.box +';\
					display				: table;\
			...
			STYLES += '\
			.blogs-Rp, .calc-Rp, .images-Rp, .maps-Rp, .news-Rp,\
			.trans-Rp, .video-Rp, .wiki-Rp, .youtube-Rp {\
				'+ CSS.res.box + ';\
				display				: table;\
				min-width			: 99%;\
				padding				: 0;\
			...
			.blogs-Rp .r, .calc-Rp > .r, .images-Rp .r, .maps-Rp .r,\
			.news-Rp > .r, .video-Rp > .r, .youtube-Rp > .r, .wiki-Rp > .r {\
				display				: block;\
				min-height			: 18px;\
				margin				: -1px;\
				padding-left		: 45px;\
			...
			.blogs-Rp .r EM, .images-Rp .r EM, .maps-Rp .r EM, .news-Rp .r EM,\
			.video-Rp .r EM, .wiki-Rp .r EM, .youtube-Rp .r EM {\
				border				: 0;\
			}\
			.blogs-Rp .ts, .images-Rp .ts, .maps-Rp .ts, .news-Rp .ts,\
			.video-Rp .ts, .wiki-Rp .ts, .youtube-Rp .ts {\
				margin				: 10px;\
			}\
			.blogs-Rp .r {\
			...
			.news-Rp TD[style*="padding-right: 10px"],\
			.news-Rp TD[style*="padding-top:5px;padding-right:10px;font-size:78%;"],\
			.news-Rp .ts > TD IMG {\
				display				: none;\
			}\
			.news-Rp .gl, .news-Rp .l {\
				display				: block;\
				float				: left;\
				margin				: 0;\
			}\
			...
			.video-Rp LI, .wiki-Rp LI, .youtube-Rp LI {\
				list-style			: none;\
			...
			' : '\
				'+ (GFX.mzBrd ? '\
					.blogs-Rp, .images-Rp, .maps-Rp, .news-Rp, .trans-Rp,\
					.video-Rp, .wiki-Rp, .youtube-Rp {\
						' + CSS.cmn.mzBrd + '\
					}\
					.blogs-Rp .r, .images-Rp .r, .maps-Rp .r,\
					.news-Rp > .r, .video-Rp > .r, .youtube-Rp > .r, .wiki-Rp > .r {\
						-moz-border-radius-topleft		:'+ (GFX.mzBrdTL - 3) +'px;\
						-moz-border-radius-topright		:'+ (GFX.mzBrdTR - 3) +'px;\
						-moz-border-radius-bottomleft	: 0;\
			...
			// Images Panel
			var imagesPanel = function()
			{
				var node = selectNode('//*[@id="res"]/descendant::a[contains(@href,"images.google.")]/ancestor::div[@class="g"]') || 
								selectNode('//*[@id="res"]/descendant::a[contains(@href,"images.google.")]/ancestor::li');
				if (!node) return;
				node.className += ' images-Rp';
				bRgtBox.appendChild(node);
				rPanelCnt++;
			};
			imagesPanel();
			...
			{
				var nodes = [];
				var nd = selectNode('//*[@id="res"]/descendant::a[contains(@href,"video.google.")]/ancestor::li');
				if(nd) nodes.push( nd );
				nodes = nodes.concat( selectAllNodes('//*[@id="res"]/descendant::a/img[contains(@src,"video.google.")]/ancestor::li') );
				if (!nodes || !nodes.length) return;
				var panel;
				var holder = $('gfx-rpanel');
				if( !(panel=$('gfx-rpanel-video')) )
				{
					panel = doc.createElement('div');
					panel.id = 'gfx-rpanel-video';
					panel.className = 'video-Rp';
					var title = doc.createElement('h3');
					title.className = 'r';
					title.innerHTML = LNG.video;
					panel.appendChild(title);
					holder.appendChild(panel);
				}
				holder = panel;
				...
				while(len--)
				{
					...
					holder.appendChild(doc.createElement('br'));
				}
			};
			videoPanel();

			// YouTube Revision
			var youtubePanel = function()
			{
				if( QUERYS.site && /youtube/i.test(QUERYS.site) ) return;
				var nodes = doc.evaluate('//*[@id="res"]/descendant::a/img[contains(@src,"img.youtube.")]/ancestor::li', doc.body, null, 7, null);

				if (!nodes || !nodes.snapshotLength) return;
				var panel;
				var holder = $('gfx-rpanel');

				if( !(panel=$('gfx-rpanel-youtube')) )
				{
					panel = doc.createElement('div');
					panel.id = 'gfx-rpanel-youtube';
					panel.className = 'youtube-Rp';
					var title = doc.createElement('h3');
					title.className = 'r';
			...
			var mapsPanel = function(){
				var node = selectNode('//*[@id="res"]/descendant::a[contains(@href,"maps.google.")]/ancestor::li');
				if (!node) return;
				node.className += ' maps-Rp';
				bRgtBox.appendChild(node);
				var el = node.getElementsByClassName('r')[0];
				node.insertBefore(el, node.firstChild);
			};
			mapsPanel();
			
			// Blogs Revision
			var blogsPanel = function()
			{
				var node = selectNode('//*[@id="res"]/descendant::a[contains(@href,"blogsearch.google.")]/ancestor::*[@class="g"]');
				if (!node) return;
				node.className += ' blogs-Rp';
				bRgtBox.appendChild(node);
			};
			blogsPanel();
			...
			{
				var node = selectNode('//*[@id="res"]/descendant::a[contains(@href,"news.google.")]/ancestor::li');
				if (!node) return;
				node.className += ' news-Rp';
				bRgtBox.appendChild(node);
			};
			newsPanel();
			
			// Wikipedia  Revision
			var wikiPanel = function()
			{
				if( QUERYS.site && /wikipedia/i.test(QUERYS.site) ) return;
				var nodes = doc.evaluate('//*[@id="res"]/descendant::a[contains(@href,".wikipedia.org/wiki")]/ancestor::li', doc.body, null, 7, null);
				if (!nodes || !nodes.snapshotLength) return;
				
				var panel;
				var holder = $('gfx-rpanel');
				if( !(panel=$('gfx-rpanel-wiki')) )
				{
					panel = doc.createElement('div');
					panel.id = 'gfx-rpanel-wiki';
					panel.className = 'wiki-Rp';
					var title = doc.createElement('h3');
					title.className = 'r';
			...
			{
				var node = selectNode('//*[@id="res"]/descendant::a[contains(@href,".google.") and contains(@href,"/translate_s")]/ancestor::li');
				if (!node) return;
				node.className += ' trans-Rp';
				bRgtBox.appendChild(node);
			};
			transPanel();
		...
		AutoPager.adjustCounter(-rPanelCnt);
	...
	// ======================
		...
		if( !HEADER )
		{
			...
			HEADER.appendChild(o);
			var o = body.getElement('form[name=gs]');
			if(o) HEADER.appendChild(o);
		...
		if(GFX.bMnuFix){
			STYLES += '\
			BODY {\
				margin-top			: '+ (HEADER.getSize().y) +'px !important;\
			}\
			';
		}
		...
		// Image Bar
		//-----------------
		if(GFX.iCntType || GFX.imgFyleType || GFX.iSafe || GFX.iSzBar) {
		
		var imgBar = doc.createElement('div');
		imgBar.id = 'gfx-imgbar';
		imgBar.innerHTML = '<b class="title">'+LNG.filters+': </b>';
		...
		{
			var o, nd, f, url;
			var iSafe = body.getElement('INPUT[name="safe"][type="hidden"]');
			if (!iSafe)
			{
				o = doc.createElement('input');
				o.name = 'safe';
				o.type = 'hidden';
				(FORM||doc.body).appendChild(o);
			...
			var links = body.getElements('A[href*="/images?"]');
			...
			o.className = 'safemode';
			o.innerHTML = '\
				<a class=' + (isSafeOff ? 'off' : 'on') + ' href="' + url + '&safe=' + (isSafeOff ? 'images' : 'off')+'" title="'+LNG.safemode+' '+(isSafeOff ? LNG.disabled : LNG.enabled)+'"></a>';
			imgBar.appendChild(o);
		...
		//----------------
		if(GFX.iSzBar) {
			var sizeBar = doc.createElement('div');
			sizeBar.className = 'sizes';

			// Image Size
			//--------------------
			function addSizeElement(target, id, value, label){
				var o = doc.createElement('input');
				o.id = id;
				o.value = value;
				o.type = 'checkbox';
				o.checked = imgsizes[value];
				o.addEventListener('click', function(ev){
					imgsizes[value] = this.checked;
					this.nextSibling.setAttribute('style', (this.checked ? '' : 'opacity:0.3; -moz-opacity:.30;') );
				}, false);
				target.appendChild(o);
				o = doc.createElement('label');
				o.setAttribute('for', id);
				o.setAttribute('style', (imgsizes[value] ? '' : 'opacity:0.3; -moz-opacity:.30;') );
				if(GFX.iSzBarIcn) {
					o.className = value;
				} else {
					o.innerHTML = label;
			...
			var imgsz = URI.query.imgsz;
			imgsz = (imgsz ? imgsz : 'icon|small|medium|large|xlarge|xxlarge|huge');
			imgsz = imgsz.split('|');
			var i=0,n;
			while ( (n=imgsz[i++]) ) { imgsizes[n] = true; }

			var iSizes = body.getElement('input[name=imgsz]');
			...
			addSizeElement(sizeBar, 'szi'	,'icon'		,LNG.icon);
			addSizeElement(sizeBar, 'szs'	,'small'	,LNG.avatars);
			addSizeElement(sizeBar, 'szm'	,'medium'	,LNG.small);
			addSizeElement(sizeBar, 'szl'	,'large'	,LNG.medium);
			addSizeElement(sizeBar, 'szx'	,'xlarge'	,LNG.large);
			addSizeElement(sizeBar, 'szxx'	,'xxlarge'	,LNG.xlarge);
			addSizeElement(sizeBar, 'szh'	,'huge'		,LNG.huge);
		...
		//--------------------
		if(GFX.iCntType) {
			var iCntType = body.getElement('select[name=imagetype]');
			if(iCntType){
				var x = iCntType.options.length;
				while(x--){
					iCntType.options[x].value=iCntType.options[x].value.replace(/.*imgtype=([a-z]*).*/ig,'$1');
				}
			} else {
				iCntType = doc.createElement('select');
				iCntType.innerHTML = '\
			...
			iCntType.name='imgtype';
			iCntType.setAttribute('onchange', 'doc.forms[0].submit();return 0;');
			sizeBar.appendChild(iCntType);
			if(URI.query.imgtype){
				var x = iCntType.options.length;
				while(x--){
			...
			o = new Element('div',{
				...
				'html' : '\
					<div class=sc-block style="width:96px">\
						<div>\
							<a href="'+url+'&amp;imgcolor=red" style="background:#c00" title="Red"></a>\
						</div>\
						<div>\
							<a href="'+url+'&amp;imgcolor=orange" style="background:#fb940b" title="Orange"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=yellow" style="background:#ff0" title="Yellow"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=green" style="background:#0c0" title="Green"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=teal" style="background:#03c0c6" title="Teal"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=blue" style="background:#00f" title="Blue"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=purple" style="background:#762ca7" title="Purple"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=pink" style="background:#ff98bf" title="Pink"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=white" style="background:#fff" title="White"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=gray" style="background:#999" title="Gray"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=black" style="background:#000" title="Black"></a>\
						</div>\
						<div>\
						<a href="'+url+'&amp;imgcolor=brown" style="background:#885418" title="Brown"></a>\
						</div>\
					</div>\
					<a href="'+url+'" style="white-space:nowrap;clear:both;padding-left:2px;color:#00c;display:block" title="All colors">'+LNG.all.toUpperCase()+' '+LNG.color.toUpperCase()+'</a>'
				}));
		...
		if(GFX.iExactSz)
		{
			...
			if(URI.query.imgw && URI.query.imgw != ''){
				var query = body.getElement('input[name=q]');
			...
			iWidth.setAttribute('size','4');
			iHeight.setAttribute('size','4');
			...
			vlist.setAttribute("onclick", "addList(this,\',,16,32,48,64,128,256,320,480,640,800,900,1024,1200,1248,1600,1856,2574,2720,2784,2832,3744,3861,4080,4256,5616\');");
		...
		if(GFX.iType)
		{
			var iFileType = body.getElement('input[name=as_filetype]');

			if (!iFileType)
			{
				o = doc.createElement('input');
				o.name = 'as_filetype';
				o.value = URI.query.as_filetype||'';
				o.type = 'hidden';
				(FORM||doc.body).appendChild(o);
			...
			var fileType = doc.createElement('select');
			fileType.className = 'filetype';
			fileType.setAttribute('onchange', 'frm=doc.forms[0];frm.as_filetype.value = this.options[this.selectedIndex].value;frm.q.value=frm.q.value.replace(/ ?filetype:[a-z0-9]*/ig,"");frm.submit();return false;');
		...
		//--------------------------
		if(QUERYS.site){
			var sd = body.getElement('td[id=sd]') || selectNode('//span[@id="sd"]/ancestor::td');
			if(sd){
				var html = '<span style="font-size:14px;"><b>' + LNG.only + ': ';
		...
		//-------------------------------------
		addSearchers( (HEADER || $('ImgCont').previousSibling), SERVICE);
	...
	else if(canRun.news)
	{
		...
		if( !HEADER ){
			...
			if(GFX.bMnuFix) HEADER.addClass('fixed');
			HEADER.appendChild(GMENU);
			var o = body.getElement('*[class=gaiaNav]');
			if(o) HEADER.appendChild(o);
			var o = doc.createElement('div');
			o.className='gbh';o.style.left='0';
			HEADER.appendChild(o);
			...
			o.className='gbh';o.style.right='0';
			HEADER.appendChild(o);
			var o = $('search-header')||$('browse-header');
			if(o) HEADER.appendChild(o);
			body.insertBefore(HEADER, body.firstChild);
		}
	
		STYLES += (isOpera ?'.gac_m { margin-top : 19px !important;}' : '');

		if(hasSomeQuery) {
			var o = selectNode("//*[@class='leftnav']/following-sibling::*");
			...
			.story {\
				'+ CSS.res.box + ';\
				max-width			: 70%;\
				width				: ' + parseInt(titlesSize*36) + 'pt;\
				padding-left		: 5pt;\
			}\
			.story SPAN.cnt {\
				'+ CSS.res.cnt + ';\
			...
			.story > A[id|="u"]:visited,\
			.story .title:visited {\
				'+ CSS.res.tit.replace(/[ ;^]color\s?:\s?[^ ;]+/,'') + ';\
			}\
			.story .body {\
				'+ CSS.res.dsc +';\
				min-width			:100%;\
			...
			if(GFX.bMnuFix) STYLES += '\
				BODY {\
					margin-top			: '+ (HEADER.getSize().y-13) +'px !important;\
				}\
				#sft * {\
					margin-bottom		: 0;\
				}\
			...
			if(GFX.bMnuFix) STYLES += '\
				BODY {\
					margin-top			: '+ (HEADER.getSize().y+15) +'px !important;\
				}\
				INPUT[name="btnmeta=search=search"] {\
					display				: none;\
				}\
		...
		// ThumbShots STYLES
		//----------------------------
		if(GFX.rThShots && hasSomeQuery) {
		
		STYLES += '\
			.story {\
				'+ CSS.res.box +';\
				width				: ' + (GFX.rThLft ? parseInt(titlesSize*36) : parseInt(titlesSize*44)) + 'pt;\
		...
		if(hasSomeQuery) {
			...
			//-------------------------------------
			addSearchers((HEADER || body.getElement('.search-sub-header')), SERVICE);

			// Auto Streaming Pages
			// -----------------------------
			AutoPager( {
				holder	: "//*[@id='main-table']/descendant::table/descendant::td[@class='search-middle']",
	...
	else if(canRun.books)
	{
		...
		if( !HEADER ){
			...
			o.className='gbh';o.style.right='0';
			HEADER.appendChild(o);
			var o = doc.getElementsByTagName('table')[0];
			if(o) {
				o.id = 'sft';
				HEADER.appendChild(o);
			}
		...
		.rsi TR {\
			'+ CSS.res.box + ';\
			display				: block;\
			width				: ' + parseInt(titlesSize*44) + 'pt;\
		}\
		.rsi B {\
			border				: 0;\
			'+ CSS.res.hl +';\
		...
		//----------------------------------
		addSearchers( (HEADER || $('results_bar')), SERVICE);

		// Auto Streaming Pages
		// -----------------------------
		AutoPager( {
			holder	: "//*[@class='scontentarea']",
			content	: "//*[@class='scontentarea']"
		} );
	...
	else if(canRun.blogsearch)
	{
		...
		.g {\
			'+ CSS.res.box +';\
			display				: block;\
			min-width			: ' + parseInt(titlesSize*50) + 'px;\
			white-space			: nowrap;\
		}\
		.g DIV.cont {\
		...
		if(GFX.bMnuFix) STYLES += '\
			BODY {\
				margin-top			: '+ (HEADER.getSize().y+32) +'px !important;\
			}\
			#header.fixed {\
		...
		//----------------------------------
		addSearchers( (HEADER || body.getElement('TABLE.ttt')), SERVICE);

		// Auto Streaming Pages
		// ----------------------------
		AutoPager( {
			holder	: "//*[@id='m']",
			content	: "//div[@class='ln']/following-sibling::*"
		} );
		...
		if(GFX.rThShots) {
			...
			.g {\
				...
				max-width			: ' + (parseInt(titlesSize*50) + parseInt(120*GFX.rThSize)) + 'px;\
			}\
			A.tn {\
				display				: inline-block;\
				position			: relative;\
	...
	else if(canRun.groups)
	{
		...
		.g {\
			'+ CSS.res.box + ';\
			display				: block;\
			max-width			: ' + parseInt(titlesSize*50) + 'px;\
		}\
		.g TABLE {\
			min-width			: 100%;\
		}\
		...
		if(GFX.bMnuFix && hasSomeQuery) STYLES += '\
			BODY {\
				margin-top			: '+ (HEADER.getSize().y+25) +'px !important;\
			}\
			#header.fixed {\
				min-height			: 100px !important;\
			}\
	...
	else if(canRun.scholar)
	{
		...
		if(GFX.bMnuFix) STYLES += '\
			BODY {\
				margin-top			: '+ (HEADER.getSize().y+30) +'px !important;\
			}\
			#header.fixed {\
				min-height			: 100px !important;\
			}\
		...
		if(GFX.rAutoPag){
			...
			var nav = body.getElement('DIV[class=n]');
			if (nav){
				doc.body.insertBefore(holder, nav);
				AutoPager( {
					holder	: "//DIV[@id='gfx-holder']",
					content	: "//P[@class='g']",
	...
	else if(canRun.products)
	{
		...
		BODY {\
			margin-top			: '+ (HEADER.getSize().y+42) +'px !important;\
		}\
		#header.fixed {\
		...
		// Remove Top Sponsors
		//----------------------------
		var ads = selectNode('//*[@id="ps-top-ads-sponsored"]');
		if(ads) ads.parentNode.parentNode.removeChild(ads.parentNode);
		
		// Add Other Products Searches
		//----------------------------------
		addSearchers( (HEADER || $('ps-titlebar')), SERVICE);

		// Auto Streaming Pages
		// -----------------------------
		var list = selectNode("//table[@class='list']");
		if(list) {
			var holder = doc.createElement('div');
			holder.id = 'gfx-holder';
			list.parentNode.insertBefore(holder, list);
			var refine = selectNode('//*[@id="ps-restrict-query"]');
			if(refine) {
	...
	else if(canRun.video)
	{
		...
		//----------------------------------
		if(hasSomeQuery) {
			addSearchers( ( $('videoheader').lastChild || $('resultsheadertable') ), SERVICE);
		}
	...
	else if(canRun.maps || canRun.local)
	{
		...
		var tb = selectNode("//*[@id='search']");
		var o = selectNode("//*[@id='guser']");
		if(o && tb) {
			HEADER.insertBefore(o, tb);
			HEADER.insertBefore(GMENU, tb);
		}
		var o = doc.createElement('div');
		o.className='gbh';o.style.left='0';
		HEADER.appendChild(o);
	...
	else if (canRun.intl || canRun.options)
	{
		...
		var o = selectNode("//td[@id='hc-links']/font");
		if(!o){ o=$('0a'); if(o) o=o.parentNode;}
		if(o) o.innerHTML = '';
	}
	...
	else if(canRun.picasaweb)
	{
		...
		var doRelink = function(){
			top.window.setTimeout(function(){
				var o = doc.evaluate("//a[@class='goog-icon-list-icon-link'][@href]", doc.body, null, 6, null);
				var i = o.snapshotLength;
				while (i--) {
					var a = o.snapshotItem(i);
		...
		doRelink();
			...
			if(GFX.iPrev) 
			{
				...
				';
				SCRIPTS += 'var iPreviewRT=' + GFX.iPrevRT + ';';
				SCRIPTS += '\
				var iPreviewID;\
				var isPreview;\
				var iPreview = doc.createElement("div");\
				iPreview.id="iPreview";\
				doc.body.appendChild(iPreview);\
				function showImg(ev,path,file){\
					...
					iPreview.style.left = (pos.x > w ? pos.x - w - 30 : pos.x + 30) + "px";\
					iPreview.style.bottom = parseInt((height - h)/2) + "px";\
					var img = doc.createElement("IMG");\
					img.onerror = function(){\
						iPreview.removeChild(img);\
						...
						iPreview.appendChild(sp);\
					};\
					img.setAttribute("width", w);\
					img.setAttribute("height", h);\
					iPreview.appendChild(img);\
					iPreviewID = window.setTimeout(function(){\
				...
				function hideImg(){\
					isPreview = false;\
					window.clearTimeout(iPreviewID);\
					if(!iPreview.firstChild) return;\
					iPreview.removeChild(iPreview.firstChild);\
	...
	if(canRun.gSuggest){
		...
		var qInput = body.getElement('input[name=q]');//selectNode("//input[@name='q']");
		...
		function doSecond() {\
			...
			function installAC(input,d,e){\
				...
				addRule("tr.gac_a:hover, tr.gac_a.hover","background-color:#479; color:white; text-shadow:0 0 2px #bbb;");\
				addRule("tr.gac_a.hover","background-color:#58a;");\
				addRule("tr.gac_a:hover td, tr.gac_a.hover td","color:white;");\
				...
				addRule(".gac_c b","font-weight:bold;");\
	...
	$(top.window).addEvent('resize',function(){
		BgBox.setStyles({
			'height':window.getScrollHeight()+'px',
			'width':window.getScrollWidth()+'px'
		});
...
{	
	...
	if(GFX.rNoTrack)
	{
		var o = doc.evaluate("//a[contains(@href,'/pagead?') or contains(@href,'/url?')]", doc.body, null, 6, null);
		var i = o.snapshotLength;
		while(i--) {
		...
		var o = doc.evaluate("//div[@id='res']/descendant::A[@onmousedown]", doc.body, null, 6, null);
		var i = o.snapshotLength;
		while(i--) {
			var el = o.snapshotItem(i);
	...
	if(canRun.search)
	{
		var results = body.getElements('DIV[id=res] LI[class*=g] H3 A');
		...
		var setThumbshot = (GFX.rThShots) ?
			function(el)
			{
				el.set('target', GFX.rTrgLnk);
				el.set('title', el.href);
		...
		if(GFX.rSiteFlt)
		{
			...
			var cites = body.getElements('DIV[class^=s] CITE');
		...
		// Reference Icons
		if(GFX.rRefIcn) {
			var links = body.getElements('LI[class*=g] A[class!=l]');
	...
	else if(canRun.images)
	{
		...
		var ref = doc.evaluate("//*[contains(@id,'tDataText')]/font/descendant::span[@class='a']", doc.body, null, 7, null);
		...
		while(x--)
		{
			...
			if(href[1]){
				...
				/*
				data.imgurl = a.href;
				data.imgrefurl = a.href.split('/');
				data.imgrefurl = data.imgrefurl.slice(0,data.imgrefurl.length-1).join('/');
				var db = rf.parentNode.innerHTML.match(/([0-9]+)\s+x\s+([0-9]+)\s+-\s+([0-9]+)/);
				if(db[1]){
					data.w  = db[1];
					data.h  = db[2];
					data.sz = db[3];
			...
			var iRefs = doc.createElement('span');
			...
			var sfUrl = data.imgurl.replace(/^https?:\/\//,'');
			sfUrl = sfUrl.split('/');
			...
			var sfUrl = data.imgrefurl.replace(/^https?:\/\//,'');
			sfUrl = sfUrl.split('/');
			sfUrl = sfUrl.slice(0,sfUrl.length-1).join('/');
	...
	{
		var o = selectNode("//*[@id='m']/div/descendant::a[contains(@href,'scoring=')]/ancestor::table");
		...
		if( $('m') )
		{
			...
			// Fix Results
			var items = body.getElements('A[id^="p-"]');
			var holder = body.getElements('P[class=g]');
			...
			var oh = holder[holder.length-1];
			if( !oh.hasChildrens )	oh.destroy();
	...
	// ==============
		...
		var o = doc.evaluate("//a[@href][not(contains(@href,'search?q=cache:'))]", doc.body, null, 6, null);
...
var GFX_Styles = function(){
	...
	//-----------------
		...
		#gfx-imgbar .safemode A:hover {\
			opacity				: 1 !important;\
		}\
		#gfx-imgbar .safemode A.on {\
		...
		#gfx-imgbar .imgc A.on {\
			border				: 1px solid #7da;\
			background-color	: #dfe;\
		}\
		#gfx-imgbar .imgc A.color {\
		...
		#gfx-imgbar .sc-block div{\
			background			: #fff;\
			float				: left;\
			font-size			: 2px;\
			margin				: 0;\
		}\
		#gfx-imgbar .sc-block div:hover{\
			background:#000;\
		}\
		#gfx-imgbar .sc-block div a{\
		...
		#gfx-imgbar .sc-block div:hover a{\
			border-color		: #fff;\
			cursor				: default;\
		}\
		#gfx-imgbar .sc-icon{\
		...
		#gfx-imgbar .sc-show div{\
			display				: block;\
		}\
		#gfx-imgbar .sc-hide div{\
			display				: none;\
		}\
		#iPreview {\
			display				: none;\
		...
		(GFX.iSzBar ? '\
			...
			#gfx-imgbar .sizes INPUT[type="checkbox"] {\
			' + (isOpera ? '\
				' : '\
				display				: none;\
			') + '\
...
var GFX_ui_Styles = function(){
	...
	#gfx-preview .res_box		{'	+ CSS.res.box	+'}\
	#gfx-preview .res_cnt		{'	+ CSS.res.cnt	+'}\
	#gfx-preview .res_tit		{'	+ CSS.res.tit	+'}\
...
var GFX_Scripts = function(){
	...
	var setTabMnu = function(n,ev) {\
		...
		lastTabNum = n;\
		if(n < 5) {\
			($("gfx-preview")).style.display="inline-block";\
		} else {\
			($("gfx-preview")).style.display="none";\
		}\
		return false;\
	};\
	window.gbar={};\
	(function(){\
		...
		isVisible = false;\
		var overID=null;\
		$o.open = function(event,id) {\
			event.cancelBubble = true;\
			var el = event.target;\
			current = $(id);\
			isOver=true;\
			if(last != null){\
				if(last != current && event.type != "click"){\
					last.style.display = "none";\
				} else if(last == current && isVisible) {\
					isVisible = false;\
					last.style.display = "none";\
					return;\
				}\
			...
			last = current;\
			el.onmouseout = function(){isOver=false;};\
			current.onmouseover = function(){isOver=true;};\
			current.onmouseout = function(){isOver=false;$o.close(400);};\
			doc.onclick = function(){isOver=false;$o.close(0);};\
			var lft = 0;\
			do{\
				lft += el.offsetLeft;\
			} while(el=el.offsetParent);\
			current.style.left = lft + "px";\
			isOver = true;\
			window.clearTimeout(overID);\
			overID=window.setTimeout(function(){\
				if(!isOver) return;\
				current.style.display = "block";\
				isVisible = true;\
			},250);\
		};\
		$o.close = function(timeout) {\
			window.clearTimeout(overID);\
			var onTimeout = function() {\
				if(isOver) return;\
				isVisible = false;\
			...
			if(timeout) overID=window.setTimeout(onTimeout,400);\
			else onTimeout();\
		};\
	})();\
	var onlyFrom = function (url,ev) {\
		...
		var found = f.q.value.indexOf(url) != -1;\
		if(found){\
			var path = url.split("/");\
			if(path[1]) {\
	...
	if (canRun.images) {
		...
		//--------------------
		if(GFX.iPrev){
			STR += 'var iPreviewRT=' + GFX.iPrevRT + ';';
			STR += '\
			var iPreviewID;\
			var isPreview=false;\
			...
			function hideImg(timeout){\
				...
				if(timeout) iPreviewID=window.setTimeout(onTimeout, timeout);\
				else onTimeout();\
			};\
			if(window.opera) \
...
var GFX, CSS, SEARCHER, LANG, isDefLng, LNG, STYLES, SCRIPTS, SERVICE, URI, QUERYS, hasSomeQuery, canRun, titlesSize, WinScroller, BgBox;
// 	Declaration of Global Functions
var addStyle, encodeURL, decodeURL, selectAllNodes, selectNode, insertAfter;
...
if(isOpera){
	...
	var OP_getValues = function(name){
		...
		var value = doc.cookie.match('(?:^|;)\\s*' + _name.escapeRegExp() + '=([^;]*)');
		if(value){
			...
			if( !data['_ver'] ){
				...
				var callback = function(value, idx){
					...
					switch(type){
						...
						case 'object':
						case 'array':
							$each( value, callback.bind(this[idx]) );
							break;
					}
				};
				$each(curData, callback.bind(data));
...
if(isFirstRun) {
	...
	var iFrmRew_Loaded = false;
	
	$(iFrmReq).addEvent('load',function(ev){
		if(iFrmRew_Loaded || _isReady) return;
		iFrmRew_Loaded = true;
		win.setTimeout(function(){
			if(_isReady) return;
			GM_log("Can't load remote language: " + iFrmReq.src);
			_isReady = true;
			GFX_ExtLang();
			GFX_FirstRun();
			GFX_byRequest();
	...
	doc.body.appendChild(iFrmReq);
	
	top.window.addEventListener('message',function(ev){
		_isReady = true;
		LNG = eval(ev.data);
		GFX_ExtLang();
		GFX_FirstRun();
	...
	//iFrmReq.src = 'http://googlefx.co.cc/lang/lng_'+ LANG +'.html?v='+ SCRIPT_VER.replace(/\./g,'_');
	iFrmReq.src = 'http://googlefx.co.cc/lang/'+SCRIPT_VER.replace(/\./g,'_')+'/lng_'+ LANG +'.html';
