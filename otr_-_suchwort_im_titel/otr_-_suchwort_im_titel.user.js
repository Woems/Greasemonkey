// ==UserScript==
// @name           OTR - Suchwort im Titel
// @namespace      Woems
// @description    Zeigt das Suchwort im Titel
...
if ($('searchterm'))
{
  document.title="OTR - search";
  if ($('searchterm').value!="") document.title=document.title+' "'+$('searchterm').value+'"';
  $('typ').options[$('typ').selectedIndex].textContent
  if ($('typ').value!="") document.title=document.title+' "Genre: '+$('typ').options[$('typ').selectedIndex].textContent+'"';
}
