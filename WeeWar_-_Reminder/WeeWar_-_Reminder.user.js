...
function headquarters(username, passwd, cb) {
  GM_xmlhttpRequest({
    method: "GET",
...
if (!config["ApiKey"])
{
  config["Name"]=prompt("WeeWar Username:");
  config["ApiKey"]=prompt("WeeWar ApiKey:");
  serialize('config',config);
}

if (!config["LastUpdate"] || config["LastUpdate"] < +new Date()-60*60*1000)
{
