// ==UserScript==
// @name           Bimmelbahn-Forum
// @namespace      Woems
// @include        http://www.boards-4you.de/wbb13/60/*
// ==/UserScript==
...
function board()
{
  var Zeilen=$x("//tbody[tr[@id='tabletitle']]/tr[td[@id='tablea']]");
  Zeilen.forEach(function (row) {
    var Link=$xs(".//a",row.cells[2]).href;
    var ID=Link.replace(/^.*threadid=([0-9]*)&sid=([a-z0-9]*)$/,"$1");
    var data=deserialize("data",{});
    if (data[ID])
    {
      if (data[ID].gut!=undefined) row.cells[2].bgColor=data[ID].gut?"lightgreen":"#FAA";
