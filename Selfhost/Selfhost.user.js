...
// @name        Selfhost
// @namespace   Woems
// @include     https://secure.selfhost.de*
// @include     http://www.selfhost.de*
// @version     1
// ==/UserScript==
...
function selfhost()
{
  ...
  {
    if ($xs('//input[@name="login_user"]')) return "login";
    return "unbekannt";
  }
  this.getPasswordFilds = function ()
  {
    return [$xs('//input[@name="login_user"]'), $xs('//input[@name="login_pass"]'), $xs('//input[@name="Domain Verwaltung anmelden"]')];
  }
}

var Selfhost=new selfhost();

switch(Selfhost.checkSite())
{
  case "login":
    var data=deserialize('data',{});
    var l=Selfhost.getPasswordFilds();
    l[0].value=data["login"]||"";
    l[1].value=data["pwd"]||"";
    window.setTimeout(function () { l[2].click(); }, 10*1000);
    l[2].addEventListener("click",function(event){
      var data=deserialize('data',{});
      data["login"]=l[0].value;
      data["pwd"]=l[1].value;
      serialize('data',data);
    }, true);
    break;
  default: alert("Seite ist: "+Selfhost.checkSite());
}
