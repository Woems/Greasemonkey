...
if ($x("//table/tbody/tr/td/a[img[@width=125]]")) // = Galerie
  $x("//table/tbody/tr/td/a[img[@width=125]]").forEach(function (a) { a.target="_blank"; });

if ($xs('//a[contains(@href,"javascript:popupWindow(")]')) // = Produkt
{
  var Link=$xs('//a[contains(@href,"javascript:popupWindow(")]');
  Link.href=Link.href.replace(/javascript:popupWindow\('(.*)'\)/,"$1");
  if ($xs('/html/body/table/tbody/tr/td/a/img'))
    Link.target="_blank";
}
if ($xs("id('tabelle')/tbody/tr/td/img")) // = Bild
{
  var img=$xs("id('tabelle')/tbody/tr/td/img");
