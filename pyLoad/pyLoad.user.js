...
var server="http://w-diskstation:8000";

//http://docs.pyload.org/module/module.Api.Api.html#module.Api.Api.generatePackages

function getQueueData(func) {
  get(server+"/api/getQueueData",function (url, text, header, xhr) {
    var ret=eval(text);
    if (func) func(ret);
    //ret.forEach(function (e) { func(e); })
    //ret.forEach(function (e) { GM_log([e.name,e.pid,e.folder,e.order,uneval(e)].join("\n")); })
  });  
} // End: function getQueueData()

function getQueue(func) {
  get(server+"/api/getQueue",function (url, text, header, xhr) {
    var ret=eval(text);
    if (func) func(ret);
    //ret.forEach(function (e) { func(e); })
    //ret.forEach(function (e) { GM_log([e.name,e.pid,e.folder,e.order,uneval(e)].join("\n")); })
...
function getCollector(func) {
  get(server+"/api/getCollector",function (url, text, header, xhr) {
    var ret=eval(text);
    if (func) func(ret);
    //ret.forEach(function (e) { func(e); })
    //ret.forEach(function (e) { GM_log([e.name,e.pid,e.folder,e.order,uneval(e)].join("\n")); })
  });  
} // End: function getCollector()

function getCollectorData(func) {
  get(server+"/api/getCollectorData",function (url, text, header, xhr) {
    var ret=eval(text);
    if (func) func(ret);
    //ret.forEach(function (e) { func(e); })
    //ret.forEach(function (e) { GM_log([e.name,e.pid,e.folder,e.order,uneval(e)].join("\n")); })
  });  
} // End: function getCollectorData()

function restartFailed(func) {
  get(server+"/api/restartFailed",function (url, text, header, xhr) {
    //GM_log([url,header,text].join("\n\n"));
    var ret=eval(text);
    if (func) func(ret);
  });  
} // End: function restartFailed()

function statusDownloads(func) {
  get(server+"/api/statusDownloads",function (url, text, header, xhr) {
    //GM_log([url,header,text].join("\n\n"));
    var ret=eval(text);
    if (func) func(ret);
  });  
...
//getCollectorData(function (e) { if (e.length>0) GM_log(e.map(function (e) { return e.order+" "+e.name+": "+Math.round(e.sizetotal/1024/1024)+" MB\n"+e.links.map(function (e) { return e.url; }).join("\n"); }).join("\n\n") ); });
//
//getCollectorData(function (e) { if (e.length>0) alert(uneval(e)); });
/*
statusDownloads(function (e) {
  if (e.length==0) aset("data", "nodownload",aget("data", "nodownload", 0)+1);
});

var nodl=aget("data", "nodownload", 0);
if (nodl>=3)
{
  aset("data", "nodownload", 0);
  getQueue(function (e) {
    if (e.length>0)
      restartFailed(function (e) { alert("Restart: "+e); });
  });
}
*/
