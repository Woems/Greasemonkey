...
function Database()
{
  var config = { baseurl:"https://woems.selfhost.eu:23882/my/wGreasemonkey/Forum.php" }
  var data={};
  var self=this;
  this.check=function (func)
  {
    ...
    get(config.baseurl+"?URL="+location.hostname, function (url, text, header, xhr) {
  ...
  {
    get(config.baseurl+"?URL="+location.hostname+"&thread="+thread, function (url, text, header, xhr) {
      data=JSON.parse(text);
      if (func) func(self, data);
    });
    return this;
  ...
  {
    get(config.baseurl+"?URL="+location.hostname+"&thread="+thread+"&HIDE="+hide, function (url, text, header, xhr) {
      data=JSON.parse(text);
      if (func) func(self, data);
    });
    return this;
  ...
  {
    get(config.baseurl+"?URL="+location.hostname+"&thread="+thread+"&READ="+read, function (url, text, header, xhr) {
      data=JSON.parse(text);
      if (func) func(self, data);
    });
    return this;
  ...
  {
    get(config.baseurl+"?URL="+location.hostname+"&thread="+thread+"&VOTE="+vote, function (url, text, header, xhr) {
      data=JSON.parse(text);
      if (func) func(self, data);
    });
    return this;
