...
{
  case '/challenge/misc/d/index.php': Dungeon(); break;
  //default: GM_log(location.pathname); break;
}

function Dungeon()
{
  css(".North:after { content:' (^)'; }");
  css(".East:after { content:' (>)'; }");
  css(".South:after { content:' (v)'; }");
  css(".West:after { content:' (<)'; }");
  if ($xs("//a[text()='try again']")) serialize('data',{});
  var data=deserialize('data',{});
  var Level=$xs("//h2").textContent;
  if (!data[Level]) data[Level] = { x:10, y:10, mace:[] };
  var Links={};
  $x("//a[contains(@href,'m=')]").forEach(function (a) {
    a.className=a.textContent;
    Links[a.textContent]=true;
    a.addEventListener("click",function(event){
      ...
      {
        case 'North': data[Level].x=data[Level].x-1; break;
        case 'East': data[Level].y=data[Level].y+1; break;
        case 'South': data[Level].x=data[Level].x+1; break;
        case 'West': data[Level].y=data[Level].y-1; break;
        default: alert(e.textContent); break;
      }
      if (!data[Level].mace[data[Level].x]) data[Level].mace[data[Level].x]=[];      
      data[Level].mace[data[Level].x][data[Level].y]=true;
      serialize('data',data);
    }, true);
  });
  if (!data[Level].mace[data[Level].x]) data[Level].mace[data[Level].x]=[];
  data[Level].mace[data[Level].x][data[Level].y]=$xs("//a[text()='Down Stairs']")?"DOWN":true;
  if (Links['North'] || Links['East'] || Links['South'] || Links['West'])
  {
    if (!Links['North']) { if (!data[Level].mace[data[Level].x-1]) data[Level].mace[data[Level].x-1]=[]; data[Level].mace[data[Level].x-1][data[Level].y]=false; }
    if (!Links['East']) { if (!data[Level].mace[data[Level].x]) data[Level].mace[data[Level].x]=[]; data[Level].mace[data[Level].x][data[Level].y+1]=false; }
    if (!Links['South']) { if (!data[Level].mace[data[Level].x+1]) data[Level].mace[data[Level].x+1]=[]; data[Level].mace[data[Level].x+1][data[Level].y]=false; }
    if (!Links['West']) { if (!data[Level].mace[data[Level].x]) data[Level].mace[data[Level].x]=[]; data[Level].mace[data[Level].x][data[Level].y-1]=false; }
  }
  css(".pos10x10 { color:red }");
  css(".pos"+data[Level].x+"x"+data[Level].y+" { color:green }");
  var text="Position: "+data[Level].x+" / "+data[Level].y+"<br>";
  text+="<table border=1>";
  for (var x = 0; x<data[Level].mace.length; x++)
  {
    text+="<tr class="+x+">";
    if (data[Level].mace[x]) for (var y = 0; y<data[Level].mace[x].length; y++)
    {
      text+="<td class=pos"+x+"x"+y+">";
      text+=(data[Level].mace[x][y]==undefined?'':data[Level].mace[x][y]=="DOWN"?"#":data[Level].mace[x][y]?'o':'x');
      text+="</td>";
    }
    text+="</tr>";
  }   
  createElement('div',{ innerHTML:text },$xs("/html/body/table/tbody/tr/td[2]"));
  serialize('data',data);
}
