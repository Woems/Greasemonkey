...
// @name        Pornhide
// @namespace   Woems
// @description Versteckt PornTitel
// @include     *
// @version     1
// ==/UserScript==
...
function setData(key, value)
{
  ...
  if (!data[location.host]) data[location.host]={}
  data[location.host][key]=value;
  serialize('data',data);
}
...
if (!data[location.host])
{
  showmsg({
    id:'default_msg',
    text:'ist "'+location.host+'" ok?',
    fixed:true,
    color:'lightgray',
    OK:'OK',
    onOK:function (e) { setData('porn',false); },
    Cancel:'x',
    onCancel:function (e) { document.title="Google"; setData('porn',true); },
    onTimeout:function () {},
    Timeout:5,
  });
} else {
  var TitelListe=["Google",'Woems Wiki'];
