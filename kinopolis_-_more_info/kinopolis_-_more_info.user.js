...
// @namespace      Woems
// @include        http://www.kinopolis.de/bn/film_details*
// ==/UserScript==
...
var Titel=$xs("id('week_1')/h2").textContent;
var MovieBox=$xs('id("moviebox")/..');
var MovieBoxSize=[536, 300];
var TextBox=$xs("id('contentRegionInner')//div[@class='moviesIndexContentViewMovie']");

TextBox.appendChild(createElement('div',{ id:"wAdvancedInfo" }));

$("wAdvancedInfo").appendChild(createElement('h3',{ textContent:"MovieMace" }));
$("wAdvancedInfo").appendChild(createElement('span',{ id:"wMM" }));
MovieMaze(Titel, function (Titel, URL) {
  $("wMM").appendChild(createElement('a',{ target:"_new", href:URL, textContent:Titel }));
  $("wMM").appendChild(createElement('br'));
});
$("wAdvancedInfo").appendChild(createElement('h3',{ textContent:"Google" }));
$("wAdvancedInfo").appendChild(createElement('a',{ target:"_new", href:"http://google.de/search?q="+Titel, textContent:Titel }));
$("wAdvancedInfo").appendChild(createElement('br'));

MovieBox.innerHTML=""
MovieBox.appendChild(iframe("http://www.youtube.com/embed?listType=search&autoplay=1&list="+Titel,"wYoutube",536,300)); // https://developers.google.com/youtube/player_parameters?hl=de-DE

//MovieMaze(Titel, function (Titel, URL) { GM_openInTab(URL); });
//MovieMaze(Titel, function (Titel, URL) { alert([Titel,URL].join("\n")); MovieBox.appendChild(createElement('a',{ style:"margin:1em; color:lightgray ", target:"_new", href:URL, textContent:Titel })); });
// MovieBox.appendChild(iframe("http://www.moviemaze.de/suche/result.phtml?searchword="+Titel+"&x=27&y=11","iframe_movimaze",536,300,"<a href=>Google</a>"));
// MovieBox.appendChild(iframe("http://google.de/search?q="+Titel,"iframe_google",536,300,"<a href=>Google</a>"));
...
/**
	@name MovieMaze
	@function
	@description MovieMaze Infos Abrufen
*/
function MovieMaze(Titel, Func) { // MovieMaze(Titel, function (Titel, URL) {});
  get("http://www.moviemaze.de/suche/result.phtml?searchword="+Titel+"&x=27&y=11",function (url, text, header, xhr) {
    var div=text2div(text);
    $x("//a[text()='Trailer']",div).forEach(function (e) {
      var Titel=$x('./preceding::td[1]/descendant::b',e)[0].innerHTML;
      //alert(["Trailer:", Titel, Func].join("\n"));
      Func(Titel, e.href.replace(/kinopolis/,"moviemaze"));
/*/
      get(e.href.replace(/kinopolis/,"moviemaze"),function (url, text, header, xhr) {
        var div=text2div(text);
        alert([url,header].join("\n"));
        var Filme=$x("//td[text()='Flash']",div).map(function (e) {
          return { 
            MB: $x('./following::td[1]',e)[0].innerHTML,
            Link: $x('./following::td[2]/a',e)[0].href.replace(/kinopolis/,"moviemaze"),
            Flagge: $x('./ancestor::tbody[1]/tr[1]/td[3]/img',e)[0].src.replace(/kinopolis/,"moviemaze"),
            Bezeichnung: $x('./ancestor::td[2]/b',e)[0].innerHTML,
            Laenge: $x('./ancestor::td[2]/text()[2]',e)[0].textContent,
          };
        });
        alert([Filme].join("\n"));
      });  
