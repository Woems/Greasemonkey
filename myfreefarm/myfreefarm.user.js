...
texte["msg1b"] = /(.*) heeft op de marktplaats\s*(\d+)x (.*?) van jou<br>\s*voor (.*?) aD gekocht\./;
...
// options panel
texte["set_CacheReset"] = "Cache reset";
texte["set_ScriptHomepage"] = "Strona skryptu";
...
texte["msgUpdate"] = "Jest nowa wersja skryptu 'Doradca Farmera'. Czy chcesz j"+pl_a+" zainstalowa"+pl_c+"?";
...
function makeDraggable(appendTo,doDragX,doDragY,dragObjId){
	...
	var dragObj = top.document.getElementById(dragObjId);
	if (help){
	...
	newdiv.addEventListener("mousedown", function(event){
		...
		var dragObj = top.document.getElementById(valMouseXY[0]);
...
//GM_log("mousemoveDrag");
	if (valMouseXY[0] != ""){
		...
		var dragObj = top.document.getElementById(valMouseXY[0]);
		var divDragHelper = top.document.getElementById("divDragHelper");
...
//GM_log("mouseupDrag");
	...
	if (valMouseXY[0] != ""){
		...
		removeElement(top.document.getElementById("divDragHelper"));
		dragObj=null;
	}
	valMouseXY = ["",0,0,0,0];
...
function getData(){
	...
	prodId["Coins"] = 0;
	prodBlock[0] = false;
	if(top.document.getElementById("coins")) prodBestand[prodId["Coins"]]=parseInt(top.document.getElementById("coins").innerHTML,10);
...
//GM_log("showPage:"+page);
    ...
    var i = 0;
    while (top.document.getElementById("kunde" + i)) {
        top.document.getElementById("kunde" + i).style.display = "none";
        i++;
    }
	closeInfoPanel();
    top.document.getElementById("einkaufszettel").style.display = "none";
    top.document.getElementById("multiframe").src = page;
    top.document.getElementById("popup_garden").style.display = "block";
    top.document.getElementById("multiframe").style.display = "block";
}
function showMarket(pid){
	if(top.document.getElementById("shopframe")){
		unsafeWindow.top.initCity(1);
		closeInfoPanel();
		unsafeWindow.top.showDiv("transp3");
		top.document.getElementById("transp3").style.visibility = "visible";
		unsafeWindow.top.showDiv("shop");
		top.document.getElementById("shop").style.visibility = "visible";
		top.document.getElementById("shopframe").src="stadt/markt.php"+(pid?"?page=1&order=p&id="+pid+"&filter=1&guild=0":"");
	} else {
...
function showMarketStand(pid){
	if(top.document.getElementById("shopframe")){
		...
		unsafeWindow.top.showDiv("shop");
		top.document.getElementById("shop").style.visibility = "visible";
		top.document.getElementById("shopframe").src="stadt/marktstand.php"+(pid?"?produkt="+pid:"");
	} else {
...
function showSGH(){
	...
	unsafeWindow.top.showDiv("shop");
	top.document.getElementById("shop").style.visibility = "visible";
	top.document.getElementById("shopframe").src="stadt/shop.php?s=1";
}
function showMessage(from,page,msg){
//GM_log("showMessage:"+from+":"+page+":"+msg);
	click(top.document.getElementById("menueimg1"));
...
function buildPreise(modeStr,appendTo){
	...
	for(var w in prodNameSort){
		...
		if (modeStr.search(prodTyp[v])!=-1) {
			...
			newtr = createElement("tr",{title:texte["wert"]+": "+kT_formatgr(prodBestand[v]*gut[v]).replace("&nbsp;"," ")},newtablebody);
		...
		if(pageId){
			...
			if($("marketcontainer").childNodes.length==1){
				...
				if (!help || (help[1]==1)) {
					...
					if (valStatistik){
						var minBeob = GM_getValue(lng+"_"+server+"_minBeob","").split("|");
						var maxBeob = GM_getValue(lng+"_"+server+"_maxBeob","").split("|");
						minBeob[pageId[1]] = minPreis;
						maxBeob[pageId[1]] = maxPreis;
						GM_setValue(lng+"_"+server+"_minBeob",minBeob.join("|"));
						GM_setValue(lng+"_"+server+"_maxBeob",maxBeob.join("|"));
					}
				}
			}
			...
			if (lng=="de") {
				...
				newdiv.addEventListener("click",function(){
					...
					var url = "http://www.mff.i24.cc/chart.php?w="+window.innerWidth+"&h="+window.innerHeight+"&t=h"+time+"&shownpc="+(valStatistikNpc?1:0)+"&clip=1&server1="+server+"&product1="+pageId[1]+"&color1=green";
...
function do_marktstand(){
	...
	if ($("produkt_preis1")){
		...
		function checkPreis() {
			...
			if (gut[currId]) {
				$("preisschild").getElementsByTagName("input")[6].style.display=(((100*currPreis>valVerkaufLimitDown*gut[currId]) && (100*currPreis<valVerkaufLimitUp*gut[currId]))?"":"none");
				newtr = createElement("tr","",newtable);
...
function closeInfoPanel(){
	try{
		top.document.getElementById("infoPanel").setAttribute("name","");
		top.document.getElementById("infoPanel").style.display = "none";
...
function do_main(){
	...
	createElement("input",{id:"GMminBeob",type:"hidden"},all);
	createElement("input",{id:"GMmaxBeob",type:"hidden"},all);
	newinput = createElement("input",{id:"GMgutBeob",type:"hidden"},all);
	newinput.addEventListener("click",function(){
		if ($("GMstatBotVersion").value>0){
		GM_setValue(lng+"_"+server+"_minBeob",$("GMminBeob").value);
		GM_setValue(lng+"_"+server+"_maxBeob",$("GMmaxBeob").value);
	...
	var windmilltimeEnd = GM_getValue(lng+"_"+server+"_"+username+"_windmilltimeEnd",0);
	...
	//Ausbau-Kosten
	var ausbau = [,[2000,21500,76000,192300],[2500,15200,32500,134500,350800],[3400,16300,50200,159600,441000],[6200,33500,76000,210500,482000],[6800,44800,128200,230600,482000],,[5200,33500,128200,230600,482000],[6800,38000,128200,230600,482000],[8100,63200,163200,303900],[9700,81600,178100,330400],[10000,85000,200000,"22","38"]];
	// Wachstumsdauer in min
	var dauer = {"37":3000,"39":4000,"24":720,"35":2000,"20":480,"5":120,"1":20,"18":90,"32":720,"33":1200,"34":800,"17":15,"26":780,"40":4800,"3":45,"8":960,"6":240,"38":960,"2":45,"36":880,"43":6800,"41":5500,"19":240,"4":90,"44":7200,"7":480,"29":950,"23":800,"21":600,"42":6200,"31":1000,"22":500,"9":120,"10":360,"11":720,"12":1440,"91":1440,"25":1000,"27":2000,"28":3000,"30":4000}
	...
	// Container Divs
	var newdiv = createElement("div",{id:"gameArea",align:"center"},all);
	...
	if(GM_getValue(lng+"_"+server+"_"+username+"_valDrag",false)){
		$("gameArea").setAttribute("style","position:absolute;top:5px;left:5px;");
	...
	if(unsafeWindow.premium=="0"){
		newdiv = createElement("div",{"id":"farmlinks"},$("garten_komplett"));
		for(var v=1;v<=unsafeWindow.farmamount;v++){
			newdiv1 = createElement("div",{"onmouseout":"hideDiv('farmtooltip"+v+"');","onmouseover":"showDiv('farmtooltip"+v+"');","id":"farmlinkitemactivate","class":"link"},newdiv,v);
			newdiv1.addEventListener("click",function(){
				unsafeWindow.top.initZones(parseInt(this.innerHTML,10)); unsafeWindow.top.showMain();
			},false);
			createElement("div",{"onmouseout":"hideDiv('farmtooltip"+v+"');","onmouseover":"showDiv('farmtooltip"+v+"');","id":"farmtooltip"+v,"style":"display: none;"},newdiv1,unsafeWindow.farmname+v);
		}
		createElement("div",{"style":"clear: both;"},newdiv);
		newdiv = createElement("div",{"id":"cityline"},$("garten_komplett"));

		newdiv1 = createElement("div",{"onmouseout":"hideDiv('citytooltip1');","onmouseover":"showDiv('citytooltip1');","style":"background: url('"+unsafeWindow._GFX+"citylinkactivate.png') no-repeat scroll left top transparent;","id":"citylineitem1","class":"link"},newdiv,1);
		newdiv1.addEventListener("click",function(){
		...
		createElement("div",{"onmouseout":"hideDiv('citytooltip1');","onmouseover":"showDiv('citytooltip1');","id":"citytooltip1","style":"display:none;"},newdiv1,unsafeWindow.cityname1);

		if(unsafeWindow.cities>1){
			newdiv1 = createElement("div",{"onmouseout":"hideDiv('citytooltip2');","onmouseover":"showDiv('citytooltip2');","style":"background: url('"+unsafeWindow._GFX+"citylinkactivate.png') no-repeat scroll left top transparent;","id":"citylineitem2","class":"link"},newdiv,2);
			createElement("div",{"onmouseout":"hideDiv('citytooltip2');","onmouseover":"showDiv('citytooltip2');","id":"citytooltip2","style":"display:none;"},newdiv1,unsafeWindow.cityname2);
			newdiv1.addEventListener("click",function(){		
				if($top("citymaincontainer").style.display=="block"){
	...
	if (lng=="de") {
				...
				if (isNaN(mode)) {
					for (var v in prodName) {
						if ((prodTyp[v]==mode) && (!prodBlock[v])) {
							...
							var url = "http://www.mff.i24.cc/chart.php?w=400&h=200&t=h"+time+"&shownpc="+(valStatistikNpc?1:0)+"&clip=1&server1="+server+"&product1="+v+"&color1=green";
							...
							newdiv.addEventListener("click",function(){
								...
								newdiv = createElement("div",{style:"position:absolute;top:0px;left:0px;height:"+window.innerHeight+"px;width:"+window.innerWidth+"px;background-color:white;color:black;z-index:99;"},all,"Lade...");
								var url = "http://www.mff.i24.cc/chart.php?w="+window.innerWidth+"&h="+window.innerHeight+"&t=h"+time+"&shownpc="+(valStatistikNpc?1:0)+"&clip=1&server1="+server+"&product1="+this.id.replace("stat","")+"&color1=green";
								newobject = createElement("object",{data:url,type:type="image/svg+xml",style:"position:absolute;top:0px;left:0px;",width:window.innerWidth+"px",height:window.innerHeight+"px"},newdiv);
								createElement("param",{name:"src",value:url},newobject);
								newimg = createElement("img",{src:"http://dqt9wzym747n.cloudfront.net/pics/close.jpg",class:"link",style:"position:absolute;top:30px;right:10px;z-index:2;"},newdiv);
								newimg.addEventListener("click",function(){removeElement(this.parentNode);},false);
					...
					var url = "http://www.mff.i24.cc/chart.php?w="+breit+"&h="+hoch+"&t=h"+time+"&shownpc="+(valStatistikNpc?1:0)+"&clip=1&server1="+server+"&product1="+mode+"&color1=green";
		...
		function sendStatData () {
			var prodStr = "";
			var minBeob = splitToFloat(GM_getValue(lng+"_"+server+"_minBeob",""),"|");
			var maxBeob = splitToFloat(GM_getValue(lng+"_"+server+"_maxBeob",""),"|");
			gutBeob = splitToFloat(GM_getValue(lng+"_"+server+"_gutBeob",""),"|");
			var c=0;
			for (var v=0;v<minBeob.length;v++){
				if ((0<minBeob[v]) && (minBeob[v]<=gutBeob[v]) && (gutBeob[v]<=maxBeob[v])) {
			...
			GM_setValue(lng+"_"+server+"_minBeob","");
			GM_setValue(lng+"_"+server+"_maxBeob","");
		}
	...
	if ($("home")) $("home").addEventListener("click",closeInfoPanel,false);
		...
		if(mode==$("infoPanel").getAttribute("name")){ closeInfoPanel(); }
			...
			break; case "options":{
				...
				for(var v=0;v<animalMove.length;v++) if(animalMove[v]){
					newinput = createElement("input",{id:"inputvalMoveAnimals"+v,type:"checkbox",class:"link",checked:valMoveAnimals[v]},newtd);
					newinput.addEventListener("click",function(){
			...
			//	><tr><td>C : Club</td><td>D : Village / Farm</td></tr>
			//	<tr><td>F : Farmi-Log</td><td>H : Help</td></tr>
			//	<tr><td>M : Market Place</td><td>N : Messages</td></tr><tr><td>O : Options</td><td>P : Profit Calculation</td></tr><tr><td>S : Seed retailer</td><td>U : Overview</td></tr><tr><td>V : Contracts</td><td>X : (next unread)<br>system message</td></tr><tr><td>1 : 1st Farm</td><td>2 : 2nd Farm</td></tr><tr><td>3 : 3rd Farm</td></tr><tr><td>\previous Message, Zone, ...</td><td>-\> : next Message, Zone, ...</td></tr></table>";
			...
			break; case "zonen":{
				...
				while (unsafeWindow.top.farmisinfo[0][++farmiNr]) {
					...
					newtd.addEventListener("click",function(){
						...
						unsafeWindow.showCart(this.getAttribute("name"));
						do_kaufliste();
					},false);
					newtd = createElement("td","",newtr);
			...
			break; case "profit":{
				...
				stallbonus[91] = [1,0.95,0.9,0.75,0.65];
				var tierfutter = new Object();
				tierfutter[9] = [1,60,120,180,216,192];
				tierfutter[10] = [3,48,72,96,108,115];
				...
				tierfutter[91] = [92,6,11,16,18,23];
				...
				function buildProfitTable(mode,perc) {
					...
					var sterne = GM_getValue(lng+"_"+server+"_"+username+"_profittable","").split("|");
					for(var v=0;v<10;v++)if(!sterne[v]){ sterne[v]=1; }
					var profit = new Array();
					var c=-1;
					for (var v=0;v<prodName.length;v++) {
						if ((prodTyp[v]=="v") && (!prodBlock[v]) && (dauer[v])) {
							profit[++c] = new Object();
							profit[c]["name"] = prodName[v];
							var grow = calcGrowTimes(60*dauer[v],86400,1-sterne[0]*0.05)*120/(unsafeWindow.top.produkt_x[v]*unsafeWindow.top.produkt_y[v]);
							profit[c]["dauer"] = calcDauer(60*dauer[v],(1-sterne[0]*0.05));
							profit[c]["punkte"] = Math.round(punkte[v]*grow);
							profit[c]["gut"] = perc*gut[v]*(unsafeWindow.top.produkt_ernte[v]-1)*grow;
							profit[c]["gutBeob"] = perc*gutBeob[v]*(unsafeWindow.top.produkt_ernte[v]-1)*grow;
						}
						if ((prodTyp[v]=="e") && (!prodBlock[v]) && (dauer[v])) {
							profit[++c] = new Object();
							profit[c]["name"] = prodName[v];
							profit[c]["level"] = sterne[sternId[v]];
							var bonus = stallbonus[v][profit[c]["level"]-1];
							profit[c]["dauer"] = 60*bonus*dauer[v];
							profit[c]["punkte"] = Math.round(1440*tiere[v][profit[c]["level"]-1]*punkte[v]*unsafeWindow.top.produkt_ernte[v]/(bonus*dauer[v]));
							profit[c]["gut"] = Math.round(1440*((tiere[v][profit[c]["level"]-1]*perc*gut[v]*unsafeWindow.top.produkt_ernte[v])-(tierfutter[v][profit[c]["level"]]*(npc[tierfutter[v][0]]?Math.min(npc[tierfutter[v][0]],gut[tierfutter[v][0]]):gut[tierfutter[v][0]])))/(bonus*dauer[v]));
							profit[c]["gutBeob"] = Math.round(1440*((tiere[v][profit[c]["level"]-1]*perc*gutBeob[v]*unsafeWindow.top.produkt_ernte[v])-(tierfutter[v][profit[c]["level"]]*(npc[tierfutter[v][0]]?Math.min(npc[tierfutter[v][0]],gut[tierfutter[v][0]]):gut[tierfutter[v][0]])))/(bonus*dauer[v]));
						}
					}
					...
					var newtd = createElement("th",{class:"link"},newtr,(5*sterne[0])+"%");
					newtd.addEventListener("click",function(){
						sterne[0] = (sterne[0]%4)+1;
						GM_setValue(lng+"_"+server+"_"+username+"_profittable",sterne.join("|"));
					...
					for (var v in profit) {
						...
						newtd = createElement("td",{id:v,title:number_format(prodBestand[prodId[profit[v]["name"]]])},newtr,profit[v]["name"]);
						...
						if (profit[v]["level"]) {
							...
							newtd.addEventListener("click",function(){
								currId = prodId[profit[this.id]["name"]];
						...
						createElement("td",{style:"text-align:right;",title:kT_format(gut[prodId[profit[v]["name"]]]).replace("&nbsp;"," ")},newtr,kT_formatgr(profit[v]["gut"]));
						createElement("td",{style:"padding-right:20px;text-align:right;",title:kT_format(gutBeob[prodId[profit[v]["name"]]]).replace("&nbsp;"," ")},newtr,kT_formatgr(profit[v]["gutBeob"]));
					}
					divInfo=null;newtable=null;newtablehead=null;newtablebody=null;newtr=null;newtd=null;
				}
			...
			break; case "formulas":{
				...
				for(var v in unsafeWindow.formulas[0])if(levelnum>=unsafeWindow.formulas[0][v][1]){
					...
					createElement("div","",newtd1,kT_formatgr(sum)+(sumPts>0?", "+number_format(sumPts)+"pts":""));
				}
			}
			}
	...
	// Profit
	var newimg = createElement("img",{id:"profitcalc",class:"link",src:"http://dqt9wzym747n.cloudfront.net/pics/buildingupdatebutton_off.png"},$("divBeraterButtons"));
	newimg.addEventListener("mouseover",function(){
		this.src = "http://dqt9wzym747n.cloudfront.net/pics/buildingupdatebutton_on.png";
	...
	newimg.addEventListener("mouseout",function(){
		this.src = "http://dqt9wzym747n.cloudfront.net/pics/buildingupdatebutton_off.png";
		$("divBeraterButtonsInfo").style.display="none";
	},false);
	newimg.addEventListener("click",function(){buildInfoPanel("profit");},false);
	
	// Preise
	newimg = createElement("img",{id:"preise",class:"link",src:"http://dqt9wzym747n.cloudfront.net/pics/money.gif",style:"background-color:#333;-moz-border-radius: 20px;"},$("divBeraterButtons"));
	newimg.addEventListener("mouseover",function(){
	...
	newimg.addEventListener("click",function(){buildInfoPanel("preise");},false);

	// alle Rezepte
	if(unsafeWindow.formulas){
		newimg = createElement("img",{id:"allformulas",class:"link",src:"http://dqt9wzym747n.cloudfront.net/pics/money.gif",style:"background-color:#333;-moz-border-radius: 20px;"},$("divBeraterButtons"));
		newimg.addEventListener("mouseover",function(){
		...
		newimg.addEventListener("click",function(){buildInfoPanel("formulas");},false);
	}
	...
	$("questline").addEventListener("mouseover",function(){
		try{
			...
			var questMenge = Math.max(0,parseInt($("questline").childNodes[1].childNodes[1].innerHTML,10)-prodBestand[questWare]);
			$("questline").getElementsByClassName("blackbox")[0].innerHTML = number_format(questMenge)+" "+prodName[questWare];
			questWare = $("questline").childNodes[2].firstChild.firstChild.getAttribute("class").replace("kp","");
			questMenge = Math.max(0,parseInt($("questline").childNodes[2].childNodes[1].innerHTML,10)-prodBestand[questWare]);
			$("questline").getElementsByClassName("blackbox")[1].innerHTML = number_format(questMenge)+" "+prodName[questWare];
		}catch(err){}
	},false);
	...
	function showNextQuest(){
		if ($("questlineopener").style.display != "block"){
			...
			$("questlineback").style.display = "block";
			...
			createElement("div",{style:"position:absolute;top:0px;left:0px;height:15px;width:15px;overflow:hidden;","class":"kp"+questData[nr][0]},newdiv);
			...
			createElement("div",{style:"position:absolute;top:0px;left:0px;height:15px;width:15px;overflow:hidden;","class":"kp"+questData[nr][2]},newdiv);
	...
	window.setTimeout(function(){ // "premium" not well set at first login
		if(unsafeWindow.premium!="1"){ 
			var newimg = createElement("img",{id:"beraterNotepad",class:"link",title:unsafeWindow.lng_t_notizen,src:"http://dqt9wzym747n.cloudfront.net/pics/guild/contract.gif",style:"width:15px;height:15px;"},$("submenuecontainer"));
	...
	function do_kaufliste(){
		var canddiv = top.document.getElementById("cartheader");
			...
			for (var zoneNr=1;zoneNr<=6;zoneNr++){
				...
				if (zoneTyp[zoneNrF]!="0") {
					if ($("updateinfo"+zoneNr)){
						...
						$("updateinfo"+zoneNr).innerHTML = texte["ausbauenFuer"]+"&nbsp;"+((typeof(ausbau[zoneTyp[zoneNrF]][help])=="number")?kT_formatgr(ausbau[zoneTyp[zoneNrF]][help]):ausbau[zoneTyp[zoneNrF]][help]+"&nbsp;"+texte["coins"]);
					}
					if($("buildingdestructbutton"+zoneNr)) {
						...
						$("zone"+zoneNr).firstChild.addEventListener("mouseover",function(){
							...
							$("buildinginfoPoints"+currZone).innerHTML = number_format(points)+"&nbsp;"+texte["punkte"];
						},false);
						//mehr Huehner
		...
		if ($("citymaincontainer").style.display=="block"){
			...
			if ($("adcolumn").style.display=="block"){
				...
				if ($("adzonetooltip1").childNodes.length==3){
					createElement("div","",$("adzonetooltip1"),"<img width='12' height='12' src='"+cand[0].getAttribute("src")+"'>/<img width='12' height='12' src='"+cand[2].getAttribute("src")+"'>&nbsp;"+kT_formatgr(parseFloat(/&nbsp;(.*)/.exec($("adzonetooltip1").firstChild.innerHTML)[1].replace(regDelimThou,"").replace(regDelimDeci,"."),10)/8));
				}
				if ($("adzonetooltip2").childNodes.length==3){
					createElement("div","",$("adzonetooltip2"),"<img width='12' height='12' src='"+cand[0].getAttribute("src")+"'>/<img width='12' height='12' src='"+cand[2].getAttribute("src")+"'>&nbsp;"+kT_formatgr(gut[0]*parseInt(/&nbsp;(.*)/.exec($("adzonetooltip2").firstChild.innerHTML)[1].replace(regDelimThou,"").replace(regDelimDeci,"."),10)/16));
					$("adzonetooltip2").firstChild.innerHTML +="&nbsp;("+kT_formatgr(gut[0])+")";
				}
				if ($("adzonetooltip3").childNodes.length==3){
					createElement("div","",$("adzonetooltip3"),"<img width='12' height='12' src='"+cand[0].getAttribute("src")+"'>/<img width='12' height='12' src='"+cand[2].getAttribute("src")+"'>&nbsp;"+kT_formatgr(gut[0]*parseInt(/&nbsp;(.*)/.exec($("adzonetooltip3").firstChild.innerHTML)[1].replace(regDelimThou,"").replace(regDelimDeci,"."),10)/16));
					$("adzonetooltip3").firstChild.innerHTML +="&nbsp;("+kT_formatgr(gut[0])+")";
				}
			}
			//Rezeptdealer
			if($("formuladealer").style.display=="block"){
	...
	function loop1() {
		...
		for (var zoneNrF=1;zoneNrF<=6*parseInt(unsafeWindow.farmamount,10);zoneNrF++){
			if ((zoneTyp[zoneNrF]!="0") && (nextTime[zoneNrF] == nie)) { nextTime[0] = 0; }
				...
				if(nextTimeWasser[zoneNrF]<nie){ nextTime[0] =  Math.min(nextTime[0],86400+nextTimeWasser[zoneNrF]); }
			}
	...
	function loop5() {
		...
		GM_setValue(lng+"_"+server+"_"+username+"_bargeld",document.getElementById("bar").innerHTML.replace(" "+texte["waehrung"],"").replace(regDelimThou,"").replace(regDelimDeci,"."));
			...
			GM_xmlhttpRequest({
				...
				onload: function(response) {
					...
					if (valLastUpdate!=serverversion) {
						var updateNode = createElement("a",{class:"link",href:"http://userscripts.org/scripts/source/66964.user.js",style:"font-weight:bold;"});
			...
			if (canda[0]) {
				...
				if (help[2] == "in") {
					if (vertraegeInIds[help[1]]) {
						...
						vertraegeIn[c]["preis"][0] = candtd[3].innerHTML.replace("&nbsp;"+texte["waehrung"],"").replace(regDelimThou,"").replace(regDelimDeci,".");
					}
				}
			}
		}
	...
	function buildAlteVertraege(mode) {
		...
		for (var v in vertraege){
			...
			for (var w in vertraege[v]["menge"]) {
				newdiv = createElement("div",{style:"line-height:16px;"},newtd,number_format(vertraege[v]["menge"][w],0) +" "+ vertraege[v]["produkt"][w]);
				produktPic(vertraege[v]["produkt"][w],newdiv);
				createElement("div","",newtd1,kT_format(vertraege[v]["preis"][w]));
				createElement("div","",newtd2,kT_formatgr(vertraege[v]["menge"][w]*vertraege[v]["preis"][w]));
...
function do_login(){
	...
	if (loc[2].search("login.php")==-1) {
		...
		newform=null;
		if (loc[2]=="login.php?start=1&paypal"){ 
			click(newinput); 
		} else {
			//login
			var Now = Math.floor((new Date()).getTime()/1000);
			...
			$("login_container").getElementsByTagName("input")[2].addEventListener("click",function(){
				...
				var currUser = $("login_container").getElementsByTagName("input")[0].value.toLowerCase();
			...
			function submit_login(accNr){
				if(logindata[accNr][0]==lng){
					...
					$("login_container").getElementsByTagName("input")[0].value=logindata[accNr][2];
					$("login_container").getElementsByTagName("input")[1].value=logindata[accNr][3];
