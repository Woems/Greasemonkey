...
var links = {
	...
	decorate: function(a, li, vi) {
		...
		if(cfg.embed_mode != 'off') {
			a.target = '_blank';
			if(!vi.status) a.addEventListener('click', this.onClick);
		}
...
var embedding = {
	...
	embed: function(li, big) {
		var site = sites[li.sid], url = site.embed(li.vid, li.t), embed;
		if(site.flash) {
			embed = ce('object', {type:'application/x-shockwave-flash',data:url});
			embed.setAttribute('allowFullScreen', 'true');
			embed.setAttribute('FlashVars', 'autostart=true');
		} else {
			embed = ce('iframe', {src:url});
	...
	show: function(li) {
		...
		if(!this.styleAdded) {
			GM_addStyle('\
				...
				#YTLT-player:not(.YTLT-player-resizing) .YTLT-embed { transition-property:width,height;transition-duration:500ms;transition-timing-function:ease; }\
...
var sites = {
	'yt':{
		...
		parse:function(url) { return /^https?:\/\/((www\.|m\.)?youtu(\.be|be\.(googleapis\.)?com).+v[=\/]|#p\/[a-z]\/.+\/|youtu\.be\/)([a-z0-9_-]+)(.*[#&\?]t=([0-9hms]+))?/i.exec(url) ? {vid:RegExp.$5, t:RegExp.$7} : null; },
	...
	'll':{
		...
		embed:function(vid) { return 'http://www.liveleak.com/e/' + vid; },
		flash:true,
