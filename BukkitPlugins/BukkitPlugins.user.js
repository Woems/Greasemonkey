...
// @name        BukkitPlugins
// @namespace   Woems
// @include     http://plugins.bukkit.org*
// @version     1
// ==/UserScript==
...
function ShowInfo() {
  ...
  //alert(uneval($x("//tr/td[@class='title']/a")));
  $x("//tr/td[@class='title']/a").forEach(function (a) {
    if (!a.getAttribute('w')) a.setAttribute('w')=true;
    var id=a.href.split('/')[4];

    a.addEventListener("click",function(event){
      var id=event.target.href.split('/')[4]
    ...
    createElement('a',{
      ...
      onClick:function (a) {
        ...
        var data=deserialize('data',{});
        if (!data[a.name]) data[a.name]={};
        data[a.name].hide=true;
        serialize('data',data);
        return true;
      }
    },a.parentNode);
    ...
    if (data[id])
    {
      if (data[id].hide) a.parentNode.parentNode.style.display="none";
      if (data[id].seen) { a.parentNode.style.backgroundColor=(Now()-data[id].seen>10*60)?"lightgreen":"lightgray"; a.textContent=a.textContent+" ("+Math.floor((Now()-data[id].seen)/60)+" min)"; }
    }
  });  
} // End: function ShowInfo()

//window.setInterval(function () { ShowInfo(); }, 3000);
window.setTimeout(function () { ShowInfo(); }, 3000);
